        ...
        if (error) {
          ...
          if (typeof result === 'object' && result.css) {
            cleaner.process(result.css.toString())
              ...
              .then((callbackResult) => {
                if (typeof callbackResult === 'object' && callbackResult.css) {
                  fs.writeFileSync(`${dist}/joomla-${name}.css`, callbackResult.css.toString(), { encoding: 'UTF-8' });
            ...
            console.log(Chalk.yellow(`${dist}/joomla-${dist} was updated.`));
          }
        }
      },
    );
...
const compileCEscss = (options, path) => {
  Promise.resolve()
