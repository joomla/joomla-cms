Joomla = window.Joomla || {}, function (t, s) {
	"use strict";

	function a(t, a) {
		var e;
		["matches", "msMatchesSelector"].some(function (t) {
	...
	s.addEventListener("joomla:updated", function (t) {
		for (var e = t && t.target ? t.target : s, c = e.querySelectorAll(".btn-group"), d = 0; d < c.length; d++) for (var l = c[d].querySelectorAll("label"), i = 0; i < l.length; i++) l[i].classList.add("btn"), i % 2 == 1 ? l[i].classList.add("btn-outline-danger") : l[i].classList.add("btn-outline-success");
		var n = e.querySelector(".btn-group label:not(.active)");
		n && n.addEventListener("click", function (t) {
			var e = s.getElementById(t.target.getAttribute("for"));
			if ("checked" !== e.getAttribute("checked")) {
				var c = a(t.target, ".btn-group").querySelector("label");
		...
		for (var r = e.querySelectorAll(".btn-group input[checked=checked]"), o = (d = 0, r.length); o > d; d++) {
			var u = r[d], L = u.id, b = e.querySelector("label[for=" + L + "]");
