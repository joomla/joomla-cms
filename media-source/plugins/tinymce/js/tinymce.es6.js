  ...
  const JoomlaTinyMCE = {
    ...
    setupEditors: (target) => {
      ...
      editors.forEach((editor) => {
        ...
        this.setupEditor(currentEditor, pluginOptions);
      });
    },
    ...
    setupEditor: (element, pluginOptions) => {
      ...
      Joomla.editors.instances[element.id] = {
        ...
        getValue: () => this.instance.getContent(),
        setValue: text => this.instance.setContent(text),
        replaceSelection: text => this.instance.execCommand('mceInsertContent', false, text),
        // Some extra instance dependent
        id: element.id,
        instance: ed,
        onSave: () => { if (this.instance.isHidden()) { this.instance.show(); } return ''; },
      };
