...
((jQuery, document) => {
  ...
  PreUpdateChecker.run = () => {
    ...
    extensions.forEach((extension) => {
      ...
      PreUpdateChecker.checkCompatibility(jQuery(extension), PreUpdateChecker.setResultView);
    });
  };
  ...
  /**
     ...
     */
  PreUpdateChecker.checkCompatibility = ($extension, callback) => {
    ...
    jQuery.getJSON(PreUpdateChecker.config.serverUrl, {
    ...
    switch (extensionData.state) {
      case PreUpdateChecker.STATE.COMPATIBLE:
        if (extensionData.compatibleVersion === extensionData.currentVersion) {
          ...
          html = `<span class="badge badge-warning">${Joomla.JText._('JYES')}(${extensionData.compatibleVersion})</span>`;
        }
        break;
