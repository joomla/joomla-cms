...
((Joomla, document) => {
   ...
   * @since   2.5
   */
  Joomla.overrider.refreshCache = (event) => {
    const element = event.target;
    const refreshStatus = document.getElementById('refresh-status');

    element.states.refreshing = true;
    ...
    Joomla.request({
      ...
      onSuccess: (response) => {
        ...
        element.states.refreshing = false;
      },
      onError: () => {
  ...
  Joomla.overrider.searchStrings = (more) => {
    ...
    const spinner = document.getElementById('overrider-spinner');
    const spinnerBtn = document.getElementById('overrider-spinner-btn');
    ...
    if (!more) {
      self.states.searchstring = formSearchString.value;
      self.states.searchtype = formSearchType !== null ? formSearchType.value : 'value';
    ...
    if (more) {
      ...
      const childrens = [].slice.call(document.querySelectorAll('#results-container div.language-results'));
    ...
    Joomla.request({
      url: `index.php?option=com_languages&task=strings.search&format=json&searchstring=${self.states.searchString}&searchtype=${self.states.searchType}&more=${more}`,
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      onSuccess: (resp) => {
   ...
   * @since   2.5
   */
  Joomla.overrider.selectString = (id) => {
