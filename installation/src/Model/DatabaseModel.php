...
class DatabaseModel extends BaseInstallationModel
{
	...
	public function initialise()
	{
		...
		if (
			(in_array($options->db_type, ['pgsql', 'postgresql']) && !preg_match('#^[a-zA-Z_][0-9a-zA-Z_$]*$#', $options->db_name))
			|| (in_array($options->db_type, ['mysql', 'mysqli']) && preg_match('#[\\\\\/\.]#', $options->db_name))
		)
		{
