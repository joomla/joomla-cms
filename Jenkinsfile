    ...
    stage("Testing PHP") {
      steps {
        ...
        parallel(
          ...
          PHP55: {
            sh 'export PHPVERSION=php55;docker-compose -f build/jenkins/docker-compose.yml run --rm --project-name=php55 test bash build/jenkins/unit-tests.sh'
          },
          PHP56: {
            sh 'export PHPVERSION=php56;docker-compose -f build/jenkins/docker-compose.yml run --rm --project-name=php56 test bash build/jenkins/unit-tests.sh'
          },
          PHP70: {
            sh 'export PHPVERSION=php70;docker-compose -f build/jenkins/docker-compose.yml run --rm --project-name=php70 test bash build/jenkins/unit-tests.sh'
          },
          PHP71: {
            sh 'export PHPVERSION=php71;docker-compose -f build/jenkins/docker-compose.yml run --rm --project-name=php71 test bash build/jenkins/unit-tests.sh'
          }
        )
      }
    }
