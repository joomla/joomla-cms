      ...
      Joomla.enqueueRequest({
        ...
        if (Array.isArray(updateInfoList)) {
          if (updateInfoList.length === 0) {
            ...
            updateInfoList.filter((u) => u.security > 0).forEach((update) =>
              text += '<div>' + Joomla.Text._('PLG_QUICKICON_EXTENSIONUPDATE_SECURITY_UPDATEFOUND').replace('%s', update.name) + '</div>',
            );
            update('danger', text);
          }
        } else {
