...
class JHTMLIcon
{
	static function create($article, $params)
	{
		...
		if ($params->get('show_icons')) {
			$text = JTHML::_('image','system/new.png', JText::_('New'), NULL, true);
		} else {
			$text = JText::_('New').'&nbsp;';
		}

		$attribs	= array('title' => JText::_('New'));
		return JTHML::_('link',JRoute::_($url), $text, $attribs);
	}
	...
	static function email($article, $params, $attribs = array())
	{
		...
		if ($params->get('show_icons')) {
			$text = JTHML::_('image','system/emailButton.png', JText::_('Email'), NULL, true);
		} else {
			$text = '&nbsp;'.JText::_('Email');
		}
		...
		$output = JTHML::_('link',JRoute::_($url), $text, $attribs);
		return $output;
	}
	...
	static function edit($article, $params, $attribs = array())
	{
		...
		$text = JTHML::_('image','system/'.$icon, JText::_('Edit'), NULL, true);
		...
		$date = JTHML::_('date',$article->created);
		...
		$button = JTHML::_('link',JRoute::_($url), $text);
	...
	static function print_popup($article, $params, $attribs = array())
	{
		...
		if ($params->get('show_icons')) {
			$text = JTHML::_('image','system/printButton.png', JText::_('Print'), NULL, true);
		} else {
