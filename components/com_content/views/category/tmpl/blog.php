...
<tr>
	<td valign="top">
	<?php if ($params->get('description_image') && $category->image) : ?>
		<img src="images/stories/<?php echo $category->image;?>" align="<?php echo $category->image_position;?>" hspace="6" alt="" />
	<?php endif; ?>
	<?php if ($params->get('description') && $category->description) : ?>
...
<tr>
	...
	<?php for ($i = 0; $i < $params->get('leading'); $i ++) : ?>
		<?php if ($i >= $category->total) : break; endif; ?>
		<div>
		<?php 
...
<?php else : $i = 0; endif; ?>
<?php if ($params->def('intro', 4) && ($i < $category->total)) : ?>
<tr>
	<td valign="top">
		...
		<?php
			...
			for ($z = 0; $z < $params->def('columns', 2); $z ++) :
				...
				<?php for ($y = 0; $y < $params->get('intro') / $params->get('columns'); $y ++) :
					if ($i <= $params->get('intro') && ($i < $category->total)) :
						$this->item =& $this->getItem($i, $params);
						echo $this->loadTemplate('item');
						$i ++;
					endif;
...
<?php endif; ?>
<?php if ($params->def('link', 4) && ($i < $category->total)) : ?>
<tr>
	<td valign="top">
