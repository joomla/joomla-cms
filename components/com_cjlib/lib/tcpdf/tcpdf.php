	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		if ($this->inxobj) {
			...
			if (isset($this->footerlen[$this->page])) {
				...
				$this->footerpos[$this->page] = isset($this->pagelen[$this->page]) ? $this->pagelen[$this->page] : 0;
			}
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				$curpos = isset($this->pagelen[$startlinepage]) ? $this->pagelen[$startlinepage] : 0;
			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					$old_bordermrk = isset($this->bordermrk[$this->page]) ? $this->bordermrk[$this->page] : 0;
					$dom = $this->closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney);
					$bordermrk = isset($this->bordermrk[$this->page]) ? $this->bordermrk[$this->page] : 0;
					if ($bordermrk > $old_bordermrk) {
