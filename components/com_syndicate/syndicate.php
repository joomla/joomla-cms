...
function syndicate() {
	...
	$live = intval( mosGetParam( $_GET, 'live', 0 ) );	
	if ( $live ) {		// menu parameters
		$_Itemid = intval( mosGetParam( $_GET, 'Itemid', $Itemid ) );
		
		// meu paramss
		$menu = new mosMenu( $database );
		$menu->load( $Itemid );
		$MenuParams 	= new JParameters( $menu->params );
		$live_bookmark = $params->def( 'live_bookmark', 0 );			
		
		// test if live bookmark is enabled for the menu page
