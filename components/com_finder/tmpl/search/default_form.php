...
<form action="<?php echo Route::_($this->query->toUri()); ?>" method="get" class="js-finder-searchform">
    ...
    <fieldset class="com-finder__search word mb-3">
        <legend class="com-finder__search-legend visually-hidden">
            <?php echo $this->text('COM_FINDER_SEARCH_FORM_LEGEND'); ?>
        </legend>
        <div class="form-inline">
            <label for="q" class="me-2">
                <?php echo $this->text('COM_FINDER_SEARCH_TERMS'); ?>
            </label>
            <div class="input-group">
                ...
                <button type="submit" class="btn btn-primary">
                    ...
                    <?php echo $this->text('JSEARCH_FILTER_SUBMIT'); ?>
                </button>
                <?php if ($this->params->get('show_advanced', 1)) : ?>
                    ...
                    <button class="btn btn-secondary" type="button" data-bs-toggle="collapse" data-bs-target="#advancedSearch" aria-expanded="<?php echo ($this->params->get('expand_advanced', 0) ? 'true' : 'false'); ?>">
                        ...
                        <?php echo $this->text('COM_FINDER_ADVANCED_SEARCH_TOGGLE'); ?></button>
                <?php endif; ?>
            </div>
        </div>
    </fieldset>
    ...
    <?php if ($this->params->get('show_advanced', 1)) : ?>
        <fieldset id="advancedSearch" class="com-finder__advanced js-finder-advanced collapse<?php if ($this->params->get('expand_advanced', 0)) {
            ...
            <legend class="com-finder__search-advanced visually-hidden">
                <?php echo $this->text('COM_FINDER_SEARCH_ADVANCED_LEGEND'); ?>
            </legend>
            <?php if ($this->params->get('show_advanced_tips', 1)) : ?>
                <div class="com-finder__tips card card-outline-secondary mb-3">
                    <div class="card-body">
                        <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_INTRO'); ?>
                        <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_AND'); ?>
                        <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_NOT'); ?>
                        <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_OR'); ?>
                        <?php if ($this->params->get('tuplecount', 1) > 1) : ?>
                            <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_PHRASE'); ?>
                        <?php endif; ?>
                        <?php echo $this->text('COM_FINDER_ADVANCED_TIPS_OUTRO'); ?>
                    </div>
                </div>
            <?php endif; ?>
