...
abstract class JHtmlQuery
{
	...
	public static function explained(FinderIndexerQuery $query)
	{
		...
		if ($query->date1)
		{
			...
			$parts[] = '<span class="query-start-date">' . JText::sprintf('COM_FINDER_QUERY_START_DATE', JText::_('COM_FINDER_QUERY_DATE_CONDITION_' . strtoupper($query->when1)), $date) . '</span>';
		}
		...
		if ($query->date2)
		{
			...
			$parts[] = '<span class="query-end-date">' . JText::sprintf('COM_FINDER_QUERY_END_DATE', JText::_('COM_FINDER_QUERY_DATE_CONDITION_' . strtoupper($query->when2)), $date) . '</span>';
		}
