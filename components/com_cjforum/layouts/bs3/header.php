				...
				if($user->guest)
				{
					...
					<form action="<?php echo JRoute::_('index.php', true, $params->get('usesecure')); ?>" method="post" id="cj-login-form" class="form-inline margin-top-10 no-margin-bottom">
						<div class="form-group">
							<input id="modlgn-username" type="text" name="username" tabindex="0" size="18" class="form-control" placeholder="<?php echo JText::_('COM_CJFORUM_USERNAME') ?>" />
							<input id="modlgn-passwd" type="password" name="password" tabindex="0" size="18" class="form-control" placeholder="<?php echo JText::_('JGLOBAL_PASSWORD') ?>" />
							
							<?php if (count($twofactormethods) > 1): ?>
							<input id="modlgn-secretkey" autocomplete="off" type="text" name="secretkey" tabindex="0" size="18" class="form-control" placeholder="<?php echo JText::_('JGLOBAL_SECRETKEY') ?>" />
							<?php endif; ?>
							
							<?php if (JPluginHelper::isEnabled('system', 'remember')) : ?>
							<div class="checkbox">
								<label for="rememberme"><input id="rememberme" type="checkbox" name="remember" value="yes"/> <?php echo JText::_('COM_CJFORUM_REMEMBER_ME') ?></label>
							</div>
							<?php endif; ?>
