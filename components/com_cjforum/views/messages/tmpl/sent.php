...
$return = base64_encode(JRoute::_('index.php?option=com_cjforum&view=messages&layout=sent', false));
$api = new CjLibApi();
?>
<div id="cj-wrapper" class="pms <?php echo $this->pageclass_sfx;?>">
	...
	<ul class="nav nav-tabs margin-bottom-5" role="tablist">
		...
		<li class="active"><a href="<?php echo JRoute::_('index.php?option=com_cjforum&view=messages&layout=sent');?>"><?php echo JText::_('COM_CJFORUM_PMS_SENT');?></a></li>
	...
	<div class="panel panel-<?php echo $this->theme;?>">
		...
		{
			$author = $this->escape($item->receiver_name);
			$profileUrl = $api->getUserProfileUrl($profileComponent, $item->receiver_id);
			$receivers = array();
			
			foreach ($item->receivers as $receiver)
			{
				$receivers[] = $api->getUserProfileUrl($profileComponent, $receiver->receiver_id, false, $this->escape($receiver->receiver_name));
			}
		?>
				...
				<div class="media clearfix">
					<div class="media-object pull-left margin-right-10 hidden-phone">
						...
						<a href="<?php echo $profileUrl;?>" class="thumbnail no-margin-bottom" title="<?php echo $author?>" data-toggle="tooltip">
							<?php echo $api->getUserAvatarImage($avatarComponent, $item->receiver_id, $item->receiver_email, 24, false, $author);?>
						</a>
						<?php else:?>
						<div class="thumbnail">
							<img alt="<?php echo $author?>" title="<?php echo $author?>" data-toggle="tooltip" 
								src="<?php echo $api->getUserAvatarImage($avatarComponent, $item->receiver_id, $item->receiever_email, 24);?>">
						</div>
						<?php endif;?>
					</div>
					<div class="media-body">
						...
						<div class="muted text-muted">
							<?php echo JText::sprintf('COM_CJFORUM_SENT_TO', implode(', ', $receivers));?> &raquo;
