...
<div class="contact<?php echo $this->pageclass_sfx?>" itemscope itemtype="http://schema.org/Person">
	...
	<!-- Check for google map -->
<?php
$app      = JFactory::getApplication();
$template = $app->getTemplate(true);

if($template->params->get('googleMap')){
	$name="";
	$address="";
	$suburb="";
	$state="";
	if($template->params->get('useAddress')){
		$address = str_replace(" ","+",$this->contact->address) . "+";
		$suburb = str_replace(" ","+",$this->contact->suburb) . "+";
		$state = str_replace(" ","+",$this->contact->state);
	}
	if($template->params->get('useName')){
		$name = str_replace(" ","+",$this->contact->name) . ",";
	}
	$width = $template->params->get('mapWidth');
	$height = $template->params->get('mapHeight');
	$api = trim($template->params->get('mapApi'));
	$topMargin = $template->params->get('maptopMargin');
	$bottomMargin = $template->params->get('mapbottomMargin');
?>
<style>
   .contact-map {
        position: relative;
		...
		margin-bottom: <?php echo $bottomMargin; ?>;
...
<div class="contact-map">
	...
	<iframe width=<?php echo $width; ?> height=<?php echo $height; ?> src="https://www.google.com/maps/embed/v1/place?key=<?php echo $api . "&q=" . $name . $address . $suburb . $state; ?>"></iframe>
