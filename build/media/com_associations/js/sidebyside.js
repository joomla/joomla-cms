...
jQuery(document).ready(($) => {
  ...
  Joomla.submitbutton = function (task) {
    ...
    if (task === 'association.cancel') {
      ...
      let reference = document.getElementById('reference-association');
      let target = document.getElementById('target-association');
      const referenceId = reference.getAttribute('data-id');
      const referenceLang = reference.getAttribute('data-language').replace(/-/, '_');
      ...
      const currentSwitcher = $('#jform_itemlanguage').val();
  ...
  $(document).on('click', '#toogle-left-panel', function () {
    const referenceHide = this.getAttribute('data-hide-reference');
    const referenceShow = this.getAttribute('data-show-reference');
  ...
  $('#target-association').on('load', function () {
    ...
    if (this.getAttribute('src') != '') {
      ...
      const targetLoadedId = $(this).contents().find('#jform_id').val() || '0';

      // Remove modal buttons on the target
      $(this).contents().find('a[href=\"#associations\"]').parent()
        .find('.btn')
        .remove();
      $(this).contents().find('#associations').find('.btn')
        .remove();
      ...
      if ($(this).contents().find('#associations').hasClass('active')) {
        ...
        $(this).contents().find('.nav-tabs').find('li')
          .first()
          .addClass('active');
        $(this).contents().find('.tab-content').find('.tab-pane')
          .first()
          .addClass('active');
      }

      // Update language field with the selected language and them disable it.
      $(this).contents().find('#jform_language').val(targetLanguage)
        .attr('disabled', '');
      ...
      // - For chosen association selectors (menus).
      let chznField = target.find(`#jform_associations_${languageCode}`);
