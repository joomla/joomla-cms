...
Date.prototype.print = function (str, dateType, translate) {
  ...
  const w = this.getLocalDay(dateType);
  const s = {};
  ...
  if (Object.prototype.toString.call(JoomlaCalLocale.localLangNumbers) === '[object Array]' && translate) { tmpDate = Date.convertNumbers(tmpDate); }
    ...
    switch (b[i]) {
      ...
      case '%B':
        for (j = 0; j < 12; ++j) {
          ...
          } else if (JoomlaCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { m = j; break; }
        }
        break;
  ...
  for (i = 0; i < a.length; ++i) {
    if (a[i].search(/[a-zA-Z]+/) != -1) {
      ...
      for (j = 0; j < 12; ++j) {
        ...
        } else if (JoomlaCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { t = j; break; }
      }
      if (t != -1) {
        if (m != -1) {
          d = m + 1;
...
Date.prototype.setJalaliFullYear = function (y, m, d) {
  ...
  const j = JalaliDate.gregorianToJalali(gy, gm + 1, gd);
  if (y < 100) y += 1300;
  j[0] = y;
  if (m != undefined) {
    if (m > 11) {
  ...
  const g = JalaliDate.jalaliToGregorian(j[0], j[1], j[2]);
