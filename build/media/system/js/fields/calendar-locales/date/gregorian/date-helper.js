!(function (Date) {
  ...
  Date.prototype.getLocalWeekNumber = function (dateType) {
    ...
    const d = new Date(this.getFullYear(), this.getMonth(), this.getDate(), 0, 0, 0);
    const DoW = d.getDay();
  ...
  Date.prototype.getLocalDayOfYear = function (dateType) {
    ...
    const now = new Date(this.getFullYear(), this.getMonth(), this.getDate(), 0, 0, 0);
    const then = new Date(this.getFullYear(), 0, 0, 0, 0, 0);
    const time = now - then;
  ...
  Date.parseFieldDate = function (str, fmt, dateType) {
    if (dateType != 'gregorian') { str = Date.toEnglish(str); }
    ...
    if (y != 0 && m != -1 && d != 0) { return new Date(y, m, d, hr, min, sec); }
    y = 0; m = -1; d = 0;
    for (i = 0; i < a.length; ++i) {
    ...
    if (m != -1 && d != 0) { return new Date(y, m, d, hr, min, sec); }
    return today;
  };
  ...
  Date.prototype.print = function (str, dateType, translate) {
    ...
    const m = this.getLocalMonth(dateType);
    const d = this.getLocalDate(dateType);
    const y = this.getLocalFullYear(dateType);
    const wn = this.getLocalWeekNumber(dateType);
    const w = this.getDay();
    const s = {};
    const hr = this.getHours();
    const pm = (hr >= 12);
    let ir = (pm) ? (hr - 12) : hr;
    const dy = this.getLocalDayOfYear(dateType);
    if (ir == 0) { ir = 12; }
    ...
    let tmpDate = str.replace(re, par => s[par] || par);
    if (Object.prototype.toString.call(JoomlaCalLocale.localLangNumbers) === '[object Array]' && dateType != 'gregorian' && translate) { tmpDate = Date.convertNumbers(tmpDate); }
