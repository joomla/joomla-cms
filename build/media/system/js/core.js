...
(function (Joomla, document) {
  ...
  Joomla.loadOptions = function (options) {
    ...
    if (!options) {
      ...
      let str; let element; let option; let
        counter = 0;
  ...
  Joomla.renderMessages = function (messages, selector, keepOld, timeout) {
    let messageContainer; let type; let typeMessages; let messagesBox; let title; let titleWrapper; let i; let messageWrapper; let
      alertClass;
  ...
  Joomla.removeMessages = function (container) {
    ...
    if (typeof window.customElements === 'object' && window.customElements.get('joomla-alert')) {
      const messages = messageContainer.querySelectorAll('joomla-alert');
      if (messages.length) {
  ...
  window.getSelectedValue = function (frmName, srcListName) {
    const srcList = document[frmName][srcListName];


    const i = srcList.selectedIndex;
  ...
  Joomla.WebComponents = function () {
    ...
    const batchCustomElements = function () {
