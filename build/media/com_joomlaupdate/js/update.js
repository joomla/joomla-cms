  ...
  const structure =
	{
	  ...
	  error(req) {
	    const message = `AJAX Loading Error: ${req.statusText}`;
...
startExtract = function () {
  ...
  const post = { task: 'startRestore' };

  this.doEncryptedAjax(post, (data) => {
    stepExtract(data);
  });
};
...
finalizeUpdate = function () {
  // Do AJAX post
  const post = { task: 'finalizeRestore', factory: window.factory };
  doEncryptedAjax(post, (data) => {
  ...
  if (typeof mixed_var === 'object') {
    ...
    if (mixed_var.hasOwnProperty) {
      for (key in mixed_var) {
        ...
        if (mixed_var.hasOwnProperty(key) === false) {
          return false;
        }
      }
    }
