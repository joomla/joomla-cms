...
const knownDirs = ['templates/site/cassiopeia', 'templates/administrator/atum'];

/**
 * Will scan all the installed extensions and rebuild the cleanUpFolders registry.
 */
const updateSettings = async (options) => {
  const extensionsScanned = await readdir(`${RootPath}/build/media_source`, {
    withFileTypes: true,
  });
...
export const recreateMediaFolder = async (options) => {
  ...
  await copy(join(RootPath, 'build/media_source'), join(RootPath, 'media'), {
    filter: filterFunc,
