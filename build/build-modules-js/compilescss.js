...
const compileFiles = (options, path) => {
  ...
  files.forEach((file) => {
    ...
    Sass.render({
      ...
      if (error) {
        ...
        const cleaner =
          postcss([autoprefixer({ add: false, browsers: options.settings.browsers })]);
        const prefixer = postcss([autoprefixer]);

        cleaner.process(result.css.toString()).then(cleaned => prefixer.process(cleaned.css))
          .then((value) => {
            fs.writeFileSync(cssFile, value.css.toString(), { encoding: 'UTF-8' });
          });
  ...
  folders.forEach((folder) => {
    Recurs(folder, ['*.min.css', '*.map', '*.js', '*.scss', '*.svg', '*.png', '*.swf']).then((filesToCompile) => {
