...
module.exports.run = (file, folder) => {
  if (file.match(/\.js$/) && file.match(/\.es6\.js$/) && !file.match(/\.w-c\.es6\.js$/)) {
    // ES6 file so we need to transpile it
    // eslint-disable-next-line no-console
    console.log(`ES20XX file trnspiled: ${file}`);
    TranspileJs.compile(file, folder);
    ...
    if (folder === 'media') {
      ...
      // Ensure that the directories exist or create them
      MakeDir.run(Path.dirname(file).replace('/src/', '').replace('\\src\\', ''));
      Fs.copyFileSync(file, file.replace('/src/', '').replace('\\src\\', '').replace('.es5.js', '.js'));
      Fs.writeFileSync(file.replace('/src/', '').replace('\\src\\', '').replace('.es5.js', '.min.js'), UglifyJS.minify(Fs.readFileSync(file, 'utf8')).code, { encoding: 'utf8' });
      // eslint-disable-next-line no-console
      console.log(`Es5 file copied/minified: ${file}`);
    }
    ...
    // eslint-disable-next-line no-console
    console.log(`Web Component file trnspiled: ${file}`);
    TranspileWc.compile(file, folder);
  }
};
