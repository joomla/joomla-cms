				...
				&& Joomla.getOptions('editor') === 'tinymce'
					...
					handleKeyPressEvent(e) {
						Object.keys(this.options).forEach((action) => {
							...
							const alt='',ctrl='',shift='';
							// console.log(e.key.toLowerCase());
							if(e.key.toLowerCase() === keyEvent)
							{
								if(Number(altKey) && e.altKey)
								alt='alt'
								if(Number(ctrlKey) && e.ctrlKey)
								ctrl='ctrl'
								if(Number(shiftKey) && e.shiftKey)
								shift='shift'
								Mousetrap.bind([alt+ctrl+shift+keyEvent],this.execCommand(e, this.options[action].selector));
							}
						});
					}
				}
