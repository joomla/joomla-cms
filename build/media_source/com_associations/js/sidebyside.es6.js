...
document
  .getElementById('reference-association')
  .addEventListener('load', ({ target }) => {
    ...
    if (targetURL) {
      ...
      document
        .getElementById('target-association')
    ...
    const content =
      target.contentDocument.body || target.contentWindow.document.body;

    // If copy button used
    if (
      content.querySelector('#jform_id') &&
    ...
    const itemLanguageOptions = document.querySelectorAll(
    ...
    if (itemLanguageOptions) {
      itemLanguageOptions.forEach((option) => {
        ...
        if (typeof parse[0] !== 'undefined') {
          ...
          const langAssociationId = content.querySelector(
            `#jform_associations_${langAssociation}_id`,
          );
          if (langAssociationId && langAssociationId.value === '') {
            const referenceAssociation = document.getElementById(
              'reference-association',
            );
            if (referenceAssociation.hasAttribute('data-no-assoc')) {
              content.querySelector(
                `#jform_associations_${langAssociation}_name`,
              ).value = referenceAssociation.getAttribute('data-no-assoc');
            }
          }
        }
      });
...
document
  ...
  .addEventListener('load', ({ target }) => {
    ...
    if (target.getAttribute('src') !== '') {
      ...
      if (associations?.classList.contains('active')) {
        content
          .querySelector('a[href="#associations"]')
      ...
      if (targetLoadedId === '0') {
        ...
        if (targetLoadedId !== targetId) {
          ...
          const languageSelector = document.querySelector(
            `#jform_itemlanguage option[value^="${targetLanguage}:${targetId}:add"]`,
          );
          if (languageSelector) {
        ...
        const referenceContent =
          ...
          reference.contentWindow.document.body;
        const languageCode = targetLanguage.replace(/-/, '_');
        const title = content.querySelector(
          `#jform_${reference.getAttribute('data-title')}`,
        ).value;

        // - For modal association selectors.
        const referenceContentId = referenceContent.querySelector(
          `#jform_associations_${languageCode}_id`,
        );
        if (referenceContentId) {
          referenceContentId.value = targetLoadedId;
        }
        const referenceContentName = referenceContent.querySelector(
          `#jform_associations_${languageCode}_name`,
        );
        ...
        const referenceContentDropdown = referenceContent.querySelector(
          `#jform_associations_${languageCode}`,
        );
        if (referenceContentDropdown) {
          referenceContentDropdown.appendChild(
            createOption(targetLoadedId, title),
          );
      ...
      const referenceLanguageCode = reference
        .getAttribute('data-language')
        .replace(/-/, '_');
      const referenceTitle = reference.getAttribute('data-title-value');

      // - For modal association selectors.
      const targetContentId = content.querySelector(
        `#jform_associations_${referenceLanguageCode}_id`,
      );
      if (targetContentId) {
        targetContentId.value = referenceId;
      }
      const targetContentName = content.querySelector(
        `#jform_associations_${referenceLanguageCode}_name`,
      );
      ...
      // - For chosen association selectors (menus).
      let chosenField = content.querySelector(
        `#jform_associations_${referenceLanguageCode}`,
      );
      if (chosenField) {
      ...
      document
        .querySelectorAll('#jform_itemlanguage option')
        .forEach((option) => {
          const parse = option.value.split(':');
          ...
          if (typeof parse[1] !== 'undefined' && parse[1] !== '0') {
            ...
            const formAssociationId = content.querySelector(
              `#jform_associations_${langAssociation}_id`,
            );
            ...
            // - For chosen association selectors (menus).
            chosenField = content.querySelector(
              `#jform_associations_${langAssociation}`,
            );
            if (chosenField) {
...
Joomla.submitbutton = (task) => {
  ...
  if (task === 'association.cancel') {
    ...
    const targetLang = document
      .getElementById('target-association')
      .getAttribute('data-language');
    const referlangInput =
      window.frames['reference-association'].document.getElementById(
        'jform_language',
      );
    ...
    window.frames['reference-association'].Joomla.submitbutton(
      `${document.getElementById('adminForm').getAttribute('data-associatedview')}.save2copy`,
    );
    ...
    const referenceLang = referenceEl
      .getAttribute('data-language')
      .replace(/-/, '_');
    const targetLang = targetEl.getAttribute('data-language').replace(/-/, '_');
    const reference =
      referenceEl.contentDocument.body ||
      referenceEl.contentWindow.document.body;
    const target =
      targetEl.contentDocument.body || targetEl.contentWindow.document.body;

    // Remove it on the reference
    // - For modal association selectors.
    const referenceAssocId = reference.querySelector(
      `#jform_associations_${targetLang}_id`,
    );
    if (referenceAssocId) {
      referenceAssocId.value = '';
    }
    const referenceAssocName = reference.querySelector(
      `#jform_associations_${targetLang}_name`,
    );
    ...
    // - For chosen association selectors (menus).
    const referenceAssoc = reference.querySelector(
      `#jform_associations_${targetLang}`,
    );
    if (referenceAssoc) {
    ...
    document
      ...
      .forEach((option) => {
        ...
        if (lang) {
          ...
          const formAssociationId = target.querySelector(
            `#jform_associations_${lang}_id`,
          );
          if (formAssociationId) {
            // - For modal association selectors.
          ...
          const formAssociation = target.querySelector(
            `#jform_associations_${lang}`,
          );
          if (formAssociation) {
            // - For chosen association selectors (menus).
    ...
    // - For modal association selectors.
    const targetAssocId = target.querySelector(
      `#jform_associations_${referenceLang}_id`,
    );
    if (targetAssocId) {
      targetAssocId.value = '';
    }
    const targetAssocName = target.querySelector(
      `#jform_associations_${referenceLang}_name`,
    );
    if (targetAssocName) {
    ...
    // - For chosen association selectors (menus).
    const targetAssoc = target.querySelector(
      `#jform_associations_${referenceLang}`,
    );
    if (targetAssoc) {
      targetAssoc.value = '';
    }
    ...
    const itemLanguageItem = document.querySelector(
    ...
    currentLangSelect.dispatchEvent(
    ...
    window.frames[`${task}-association`].Joomla.submitbutton(
      `${document.getElementById('adminForm').getAttribute('data-associatedview')}.apply`,
    );
  }
