...
((Joomla, document) => {
  ...
  const handleError = (message) => {
    try {
      document.getElementById(
        'plg_multifactorauth_webauthn_validate_button',
      ).style.disabled = 'null';
    } catch (_e) {
      // Do nothing
    }
  ...
  const setUp = (e) => {
    ...
    if (!('credentials' in navigator)) {
      displayError(
        Joomla.Text._('PLG_MULTIFACTORAUTH_WEBAUTHN_ERR_NOTAVAILABLE_HEAD'),
      );

      return false;
    }

    const rawPKData = document.forms['com-users-method-edit'].querySelectorAll(
      'input[name="pkRequest"]',
    )[0].value;
    ...
    navigator.credentials.create({ publicKey }).then(
      (data) => {
        ...
        document.getElementById('com-users-method-code').value = btoa(
  ...
  const validate = () => {
    ...
    navigator.credentials.get({ publicKey }).then(
      (data) => {
        ...
        document.getElementById('users-mfa-code').value = btoa(
          JSON.stringify(publicKeyCredential),
        );
  ...
  const onValidateClick = (event) => {
    ...
    document.getElementById('users-mfa-captive-button-submit').style.disabled =
      'disabled';
    validate();
  ...
  document.getElementById('multifactorauth-webauthn-missing').style.display =
    'none';

  if (typeof navigator.credentials === 'undefined') {
    document.getElementById('multifactorauth-webauthn-missing').style.display =
      'block';
    document.getElementById('multifactorauth-webauthn-controls').style.display =
      'none';
  }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    if (Joomla.getOptions('com_users.pagetype') === 'validate') {
      document
        .getElementById('users-mfa-captive-button-submit')
        .addEventListener('click', onValidateClick);
    } else {
      document
        .querySelectorAll('.multifactorauth_webauthn_setup')
        .forEach((btn) => {
