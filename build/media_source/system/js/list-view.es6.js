 ...
 */
const setup = ({ target }) => {
  target.querySelectorAll('.js-grid-item-check-all').forEach((element) => {
    element.addEventListener('click', (event) => Joomla.checkAll(event.target));
  });
  target.querySelectorAll('.js-grid-item-is-checked').forEach((element) => {
