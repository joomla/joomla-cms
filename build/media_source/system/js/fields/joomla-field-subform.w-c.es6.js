    ...
    constructor() {
      ...
      if (this.buttonAdd || this.buttonRemove) {
        this.addEventListener('click', (event) => {
          ...
          if (that.buttonAdd) {
            btnAdd = event.target[matchesFn](that.buttonAdd)
              ? event.target
              : closest(event.target, that.buttonAdd);
          }

          if (that.buttonRemove) {
            btnRem = event.target[matchesFn](that.buttonRemove)
              ? event.target
              : closest(event.target, that.buttonRemove);
          }

          // Check actine, with extra check for nested joomla-field-subform
          if (btnAdd && closest(btnAdd, 'joomla-field-subform') === that) {
            let row = closest(btnAdd, 'joomla-field-subform');
            row = closest(row, that.repeatableElement) === that ? row : null;
            that.addRow(row);
        ...
        this.addEventListener('keydown', (event) => {
          ...
          if ((isAdd || isRem) && closest(event.target, 'joomla-field-subform') === that) {
            let row = closest(event.target, 'joomla-field-subform');
            row = closest(row, that.repeatableElement) === that ? row : null;
            if (isRem && row) {
              that.removeRow(row);
            } else if (isAdd) {
