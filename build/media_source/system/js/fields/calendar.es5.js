	...
	JoomlaCalendar.prototype._create = function () {
		...
		var cell = null,
			...
			hh   = function (text, cs, navtype, node, styles, classes, attributes) {
				...
				if (navtype !== 0 && Math.abs(navtype) <= 2) {
					cell.innerHTML = Joomla.sanitize("<a " + classes + " style='display:inline;padding:2px 6px;cursor:pointer;text-decoration:none;' unselectable='on'>" + text + "</a>");
				} else {
					cell.innerHTML = cs ? Joomla.sanitize("<div unselectable='on'" + classes + ">" + text + "</div>") : Joomla.sanitize(text);
					if (!cs && classes) {
		...
		if (this.params.weekNumbers) {
			...
			cell.innerHTML = Joomla.sanitize(JoomlaCalLocale.wk);
		}
		for (var i = 7; i > 0; --i) {
			cell = createElement("td", row);
			if (!i) {
		...
		for (var i = 0; i < 7; ++i) {
			...
			cell.innerHTML = Joomla.sanitize(JoomlaCalLocale.shortDays[(i + fdow) % 7]);
	...
	JoomlaCalendar.prototype.processCalendar = function () {
		...
		for (var i = 0; i < monthDays; ++i, row = row.nextSibling) {
			...
			if (this.params.weekNumbers) {
				...
				cell.innerHTML = Joomla.sanitize(date.getLocalWeekNumber(this.params.dateType)); //date.convertNumbers();
				cell = cell.nextSibling;
			}
			...
			for (var j = 0; j < totalDays; ++j, cell = cell.nextSibling, date.setLocalDate(this.params.dateType, iday + 1)) {
				...
				cell.disabled = false;
				cell.innerHTML = this.params.debug ? Joomla.sanitize(iday) : Joomla.sanitize(Date.convertNumbers(iday));          // translated day number for each cell
				if (!cell.disabled) {
