if (window.innerWidth > 1024) {
  ...
  let storage = {
    ...
    getData() {
      let item = window.localStorage.getItem(this.pagens());
    ...
    has() {
      return window.localStorage.hasOwnProperty(this.pagens());
    },
    set(i, v) {
  ...
  const lis = headers.forEach((el, index) => {
    if (index === 0/*checkbox*/ || index === 1/*ordering*/ || index === headers.length - 2) return;
