...
Joomla.MediaManager.Edit.Reset = (current) => {
  ...
  links.forEach((link) => {
    ...
    Joomla.MediaManager.Edit[link.getAttribute('aria-controls').replace('attrib-', '')].Deactivate();
    ...
    activate(link.getAttribute('aria-controls').replace('attrib-', ''), data);
  });
};
...
customElements.whenDefined('joomla-tab').then(() => {
  ...
  links.forEach((link) => {
    link.addEventListener('joomla.tab.shown', ({ relatedTarget, target }) => {
      const container = document.getElementById('media-manager-edit-container');
      if (relatedTarget) {
        Joomla.MediaManager.Edit[relatedTarget.id.replace('attrib-', '')].Deactivate();
      ...
      // Move the container to the correct tab
      relatedTarget.insertAdjacentElement('beforeend', container);
