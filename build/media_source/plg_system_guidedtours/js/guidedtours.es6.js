...
Joomla.guidedTour = Joomla.guidedTour || {
  emptyStorage: () => {
  ...
  getTourInstance: () => {
    ...
    tour.on('cancel', () => {
      Joomla.guidedTour.emptyStorage();
  ...
  addProgressIndicator: (stepElement, index, total) => {
    ...
    const step = new Shepherd.Step(tour, {
      ...
      when: {
        show() {
          ...
          Joomla.guidedTour.addProgressIndicator(element, this.id + 1, sessionStorage.getItem('stepCount'));
          ...
          if (target && this.options.attachTo.type === 'interactive') {
            ...
            if (target.tagName.toLowerCase() === 'iframe') {
              ...
              target.contentWindow.document.body.addEventListener('blur', (event) => {
                setTimeout(() => {
                  Joomla.guidedTour.setFocus(primaryButton, secondaryButton, cancelButton);
                }, 1);
                event.preventDefault();
              });
    ...
    tour.addStep({
      ...
      when: {
        show() {
          ...
          Joomla.guidedTour.addProgressIndicator(this.getElement(), 1, sessionStorage.getItem('stepCount'));
        },
      },
    });
  },
  ...
  startTour: (obj) => {
    ...
    // Start tour building
    const tour = Joomla.guidedTour.getTourInstance();
    ...
    if (ind < 0) {
      ...
      // Show info
      Joomla.guidedTour.showTourInfo(tour, obj.steps[0]);
      ind = 1;
    }
    ...
    for (let index = ind; index < len; index += 1) {
      ...
      if (prevStep === null || index > ind || obj.steps[index].type === 'redirect') {
        Joomla.guidedTour.addBackButton(buttons, obj.steps[index]);
      }
                ...
                if (ele.hasAttribute('required') && ['email', 'password', 'search', 'tel', 'text', 'url'].includes(ele.type)) {
                  ['input', 'focus'].forEach((eventName) => ele.addEventListener(eventName, (event) => {
                    if (event.target.value.trim().length) {
                      Joomla.guidedTour.enableButton(event);
                    } else {
                      Joomla.guidedTour.disableButton(event);
                    }
                  }));
                }
                break;
      ...
      if (index < len - 1) {
        if (
          ...
          Joomla.guidedTour.pushNextButton(buttons, obj.steps[index]);
        }
      } else {
        Joomla.guidedTour.pushCompleteButton(buttons);
      }

      Joomla.guidedTour.addStepToTourButton(tour, obj.steps[index], buttons);
    ...
    if (tourId > 0) {
      ...
      fetch(url)
        ...
        .then((result) => {
          ...
          Joomla.guidedTour.startTour(result.data);
        })
        .catch((error) => {
...
document.querySelector('body').addEventListener('click', (event) => {
  ...
  Joomla.guidedTour.loadTour(event.target.getAttribute('data-id'));
});
...
if (tourId > 0 && sessionStorage.getItem('tourToken') === String(Joomla.getOptions('com_guidedtours.token'))) {
  Joomla.guidedTour.loadTour(tourId);
} else {
  Joomla.guidedTour.emptyStorage();
}
