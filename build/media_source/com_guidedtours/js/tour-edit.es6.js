...
(() => {
  ...
  ['joomla:showon-show', 'joomla:showon-hide'].forEach((eventType) => {
    document.addEventListener(eventType, () => {
      document
        .querySelectorAll('#guidedtour-dates-form fieldset')
        .forEach((fieldset) => {
          ...
          const visibleChildren = fieldset.querySelectorAll(
            ':scope .control-group:not(.hidden)',
          );
      ...
      document
        .querySelectorAll('#guidedtour-dates-form joomla-tab-element')
        .forEach((tabelement) => {
          const tabLabel = document.querySelector(
            `button[aria-controls="${tabelement.id}"]`,
          );
          if (tabLabel) {
            const visibleChildren = tabelement.querySelectorAll(
              ':scope .control-group:not(.hidden)',
            );
            if (visibleChildren.length) {
