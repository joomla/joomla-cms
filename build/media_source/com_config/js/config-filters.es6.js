...
const recursiveApplyChanges = (id) => {
  document
    .querySelectorAll(`#filter-config select[data-parent="${id}"]`)
    .forEach((child) => {
...
const applyChanges = (event) => {
  ...
  const currentFilter =
    currentElement.options[currentElement.selectedIndex].value;

  if (currentFilter === 'NONE') {
    const childs = document.querySelectorAll(
      `#filter-config select[data-parent="${currentElement.dataset.id}"]`,
    );
    if (
      childs.length &&
      window.confirm(Joomla.Text._('COM_CONFIG_TEXT_FILTERS_NOTE'))
    ) {
