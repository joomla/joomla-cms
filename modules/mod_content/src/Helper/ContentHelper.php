...
namespace Joomla\Module\Content\Site\Helper;
...
class ContentHelper implements DatabaseAwareInterface
{
    ...
    public function getArticles(Registry $params, SiteApplication $app)
    {
        ...
        foreach ($items as &$item) {
            ...
            if ($show_introtext) {
                $item->introtext = HTMLHelper::_('content.prepare', $item->introtext, '', 'mod_content.content');
            }
        ...
        if ($items && $grouped) {
            switch ($article_grouping) {
                case 'year':
                case 'month_year':
                    $items = ContentHelper::groupByDate(
                        $items,
                ...
                case 'author':
                case 'category_title':
                    $items = ContentHelper::groupBy($items, $article_grouping, $article_grouping_direction);
                    break;
                case 'tags':
                    $items = ContentHelper::groupByTags($items, $article_grouping_direction);
                    break;
            }
        }
     ...
     *              Example: Factory::getApplication()->bootModule('mod_content', 'site')
     *                           ->getHelper('ContentHelper')
    ...
    public static function groupByTags($list, $direction = 'ksort')
    {
        ...
        if ($untagged) {
            $grouped['MOD_CONTENT_UNTAGGED'] = $untagged;
        }
