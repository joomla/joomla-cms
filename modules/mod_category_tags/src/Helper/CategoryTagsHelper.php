...
//naspace Joomla\Module\TagsPopular\Site\Helper;
...
abstract class CategoryTagsHelper
{
	...
	public static function getList(&$params)
	{
		...
		if($catid){
			...
			$query->whereIn($db->quoteName('cat.access'), $groups);// array_merge((array)$groups, [0]) // ------------------- // -------------------
//			$query->where($db->quoteName('cat.access') . " IN (". implode(',', $groups).")");// array_merge((array)$groups, [0]) // ------------------- // -------------------
...
//toPrint($query->bindArray($groups),'$query->bindArray($groups)',0,'pre', true);
//toPrint($query->bindArray($groups),'$query->bindArray($groups)',0,'pre', true);
//toPrint($query->bindArray($groups),'$query->bindArray($groups)',0,'pre', true);
//toPrint($query->bindArray($groups),'$query->bindArray($groups)',0,'pre', true);
...
//			->where($db->quoteName('c.core_access') . " IN (". implode(',', $groupIn).")");
//toPrint($groupIn,'ITEM TAG',0,'message', true);
...
//toPrint($query->bindArray($groups),'$query->bindArray($groups)',0,'pre', true);
//toPrint($groups,'$groups',0,'pre', true);
//toPrint($query->getBounded(),'$query->getBounded()',0,'pre', true);
//toPrint($query->getBounded(),'$query->getBounded()',0,'pre', true);
...
//toPrint($query,'ITEM TAG',0,'message', true);
		$db->setQuery($query);
//toPrint($query,'ITEM TAG',0,'message', true);

//toPrint((string)$query,'ITEM TAG',0,'message', true);
//toPrint($query,'ITEM TAG',0,'message', true);
		try
		{
...
//toPrint($results,'ITEM TAG',0,'message', true);
