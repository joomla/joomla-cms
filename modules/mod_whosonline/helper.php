...
class modWhosonlineHelper
{
	...
	public static function getOnlineUserNames($params)
	{
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$query->select($db->quoteName(array('a.username', 'a.time', 'a.userid', 'a.usertype', 'a.client_id')));
		$query->from('#__session AS a');
		$query->where($db->quoteName('a.userid') . ' != 0');
		$query->where($db->quoteName('a.client_id') . ' = 0');
		$query->group($db->quoteName(array('a.username', 'a.time', 'a.userid', 'a.usertype', 'a.client_id')));
