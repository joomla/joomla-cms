...
class JMainMenu extends JTree
{
	...
	function renderLF($id, $suffix = null)
	{
		global $mainframe;
		...
		while ($this->_current->hasChildren())
		{
			...
			{
				$this->_current = & $child;
				$this->renderLevelLF($suffix, $depth);
			}
			echo "</ul>\n";
		}
	}

	function renderLevelLF($suffix = null, $depth)
	{
		$depth++;
		if (!isset($this->_depthHash[$depth])) {
			$this->_depthHash[$depth] = 0;
	...
	function renderHF($id, $suffix = null)
	{
		global $mainframe;
	...
	function renderLevelHF($suffix = null, $depth)
	{
		$depth++;
		if (!isset($this->_depthHash[$depth])) {
			$this->_depthHash[$depth] = 0;
		...
		if (($depth < $end) || ($end == 0) && $showChildren) {
			...
			while ($this->_current->hasChildren())
			{
				...
				{
					$this->_current = & $child;
					$this->renderLevelHF($suffix, $depth);
				}
				if (($depth >= $start) || ($start == 0)) {
					echo "</ul>\n";
				}
