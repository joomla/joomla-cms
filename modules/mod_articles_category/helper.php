...
abstract class ModArticlesCategoryHelper
{
	 ...
	 * @param   string  $date_field                  date field to group by
	 *
	 * @return  array
	 *
	 * @since   1.6
	 */
	public static function groupByDate($list, $type = 'year', $article_grouping_direction, $month_year_format = 'F Y', $date_field = 'created')
	{
		$grouped = array();
		...
		foreach ($list as $key => $item)
		{
			...
			{
				case 'month_year' :
					$month_year = StringHelper::substr($item->$date_field, 0, 7);
				...
				case 'year' :
				default:
					$year = StringHelper::substr($item->$date_field, 0, 4);
