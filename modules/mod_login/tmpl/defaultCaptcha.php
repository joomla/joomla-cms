...
jimport('joomla.form.form'); 
/*
$formpath=JPATH_SITE.DS.'components'.DS.'com_users'.DS.'models'.DS.'forms'.DS.'login.xml'; 
//var_dump($formpath);
$form = &JForm::getInstance('mod_login',$formpath);
var_dump($form);
*/
?>
<?php if ($type == 'logout') : ?>
...
<?php foreach ($form->getFieldset('acaptcha') as $name => $field): ?>
				<label for="modlgn-captcha"><?php echo $field->label; ?></label>
	...
	<?php endif; ?>
	<button type="submit" name="Submit" class="button validate2"><?php echo JText::_('JLOGIN') ?>"</button>
	<input type="hidden" name="option" value="com_users" />
