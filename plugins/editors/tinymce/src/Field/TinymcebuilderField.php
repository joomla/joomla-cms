...
class TinymcebuilderField extends FormField
{
    ...
    protected function getLayoutData()
    {
        ...
        $menus = ['edit'   => ['label' => 'Edit'], 'insert' => ['label' => 'Insert'], 'view'   => ['label' => 'View'], 'format' => ['label' => 'Format'], 'table'  => ['label' => 'Table'], 'tools'  => ['label' => 'Tools'], 'help'   => ['label' => 'Help']];
        ...
        if (!empty($paramsAll->html_width) && empty($paramsAll->configuration['setoptions'])) {
            ...
            if (\is_object($plugin) && !empty($plugin->params)) {
                $setParams = (object) json_decode((string) $plugin->params, null, 512, JSON_THROW_ON_ERROR);
            }
        }
