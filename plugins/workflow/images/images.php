        ...
        foreach ($pks as $pk) {
            ...
            if ($introImageRequired && $resizeIntroImage) {
                ...
                if (!getimagesize($introImagePath)) {
                    ...
                    return true;
                } elseif (($model->getItem($pk)->images['image_intro'])) {
                    $image = new Image();
                    $image->loadFile($introImagePath);
                    if (!$introWidth) {
                        $introWidth = getimagesize($introImagePath)[0];
                    }
                    if (!$introHeight) {
                        $introHeight = getimagesize($introImagePath)[1];
                    }
            ...
            if ($fullArticleImageRequired && $resizeFullArticleImage) {
                ...
                if (!getimagesize($fullArticleImagePath)) {
                    return true;
                } elseif (($model->getItem($pk)->images["image_fulltext"])) {
                    $image = new Image();
                    $image->loadFile($fullArticleImagePath);
                    if (!$fullArticleWidth) {
                        $fullArticleWidth = getimagesize($fullArticleImagePath)[0];
                    }
                    if (!$fullArticleHeight) {
                        $fullArticleHeight = getimagesize($fullArticleImagePath)[1];
                    }
