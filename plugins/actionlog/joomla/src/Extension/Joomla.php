...
final class Joomla extends ActionLogPlugin
{
     ...
     * @since  3.9.0
     */
    protected array $contextAliases = ['com_content.form' => 'com_content.article'];
    ...
    public function onContentAfterSave($context, $article, $isNew): void
    {
        ...
        $message = ['action'   => $isNew ? 'add' : 'update', 'type'     => $params->text_prefix . '_TYPE_' . $params->type_title, 'id'       => $id, 'title'    => $article->get($params->title_holder), 'itemlink' => ActionlogsHelper::getContentTypeLink($option, $contentType, $id, $params->id_holder, $article)];
    ...
    public function onContentAfterDelete($context, $article): void
    {
        ...
        $message = ['action' => 'delete', 'type'   => $params->text_prefix . '_TYPE_' . $params->type_title, 'id'     => $id, 'title'  => $article->get($params->title_holder)];
    ...
    public function onContentChangeState($context, $pks, $value)
    {
        ...
        foreach ($pks as $pk) {
            $message = ['action'      => $action, 'type'        => $params->text_prefix . '_TYPE_' . $params->type_title, 'id'          => $pk, 'title'       => $items[$pk]->{$params->title_holder}, 'itemlink'    => ActionlogsHelper::getContentTypeLink($option, $contentType, $pk, $params->id_holder, null)];
     ...
     *
     * @param Registry $config Registry object with the new config
     *
     *
     * @since   3.9.0
     */
    public function onApplicationAfterSave($config): void
    {
        ...
        $message = ['action'         => $action, 'type'           => 'PLG_ACTIONLOG_JOOMLA_TYPE_APPLICATION_CONFIG', 'extension_name' => 'com_config.application', 'itemlink'       => 'index.php?option=com_config'];
    ...
    public function onExtensionAfterInstall($installer, $eid)
    {
        ...
        $message = ['action'         => 'install', 'type'           => 'PLG_ACTIONLOG_JOOMLA_TYPE_' . $extensionType, 'id'             => $eid, 'name'           => (string) $manifest->name, 'extension_name' => (string) $manifest->name];
    ...
    public function onExtensionAfterUpdate($installer, $eid)
    {
        ...
        $message = ['action'         => 'update', 'type'           => 'PLG_ACTIONLOG_JOOMLA_TYPE_' . $extensionType, 'id'             => $eid, 'name'           => (string) $manifest->name, 'extension_name' => (string) $manifest->name];
    ...
    public function onExtensionAfterSave($context, $table, $isNew): void
    {
        ...
        $message = ['action'         => $isNew ? 'add' : 'update', 'type'           => 'PLG_ACTIONLOG_JOOMLA_TYPE_' . $params->type_title, 'id'             => $table->get($params->id_holder), 'title'          => $table->get($params->title_holder), 'extension_name' => $table->get($params->title_holder), 'itemlink'       => ActionlogsHelper::getContentTypeLink($option, $contentType, $table->get($params->id_holder), $params->id_holder)];
    ...
    public function onExtensionAfterDelete($context, $table): void
    {
        ...
        $message = ['action' => 'delete', 'type'   => 'PLG_ACTIONLOG_JOOMLA_TYPE_' . $params->type_title, 'title'  => $table->get($params->title_holder)];
    ...
    public function onUserAfterSave($user, $isnew, $success, $msg): void
    {
        ...
        $message = ['action'      => $action, 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user['id'], 'title'       => $user['name'], 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user['id'], 'userid'      => $userId, 'username'    => $username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $userId];
    ...
    public function onUserAfterDelete($user, $success, $msg): void
    {
        ...
        $message = ['action'      => 'delete', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user['id'], 'title'       => $user['name']];
    ...
    public function onUserAfterSaveGroup($context, $table, $isNew): void
    {
        ...
        $message = ['action'      => $action, 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER_GROUP', 'id'          => $table->id, 'title'       => $table->title, 'itemlink'    => 'index.php?option=com_users&task=group.edit&id=' . $table->id];
    ...
    public function onUserAfterDeleteGroup($group, $success, $msg): void
    {
        ...
        $message = ['action'      => 'delete', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER_GROUP', 'id'          => $group['id'], 'title'       => $group['title']];
    ...
    public function onUserAfterLogin($options)
    {
        ...
        $message = ['action'      => 'login', 'userid'      => $loggedInUser->id, 'username'    => $loggedInUser->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $loggedInUser->id, 'app'         => 'PLG_ACTIONLOG_JOOMLA_APPLICATION_' . $this->getApplication()->getName()];
    ...
    public function onUserLoginFailure($response)
    {
        ...
        $message = ['action'      => 'login', 'id'          => $loggedInUser->id, 'userid'      => $loggedInUser->id, 'username'    => $loggedInUser->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $loggedInUser->id, 'app'         => 'PLG_ACTIONLOG_JOOMLA_APPLICATION_' . $this->getApplication()->getName()];
    ...
    public function onUserLogout($user, $options = [])
    {
        ...
        $message = ['action'      => 'logout', 'id'          => $loggedOutUser->id, 'userid'      => $loggedOutUser->id, 'username'    => $loggedOutUser->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $loggedOutUser->id, 'app'         => 'PLG_ACTIONLOG_JOOMLA_APPLICATION_' . $this->getApplication()->getName()];
    ...
    public function onUserAfterRemind($user)
    {
        ...
        $message = ['action'      => 'remind', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user->id, 'title'       => $user->name, 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'userid'      => $user->id, 'username'    => $user->name, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id];
    ...
    public function onAfterCheckin($table)
    {
        ...
        $message = ['action'      => 'checkin', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user->id, 'title'       => $user->username, 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'userid'      => $user->id, 'username'    => $user->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'table'       => str_replace($this->getDatabase()->getPrefix(), '#__', $table)];
    ...
    public function onAfterLogPurge($group = '')
    {
        $context = $this->getApplication()->input->get('option');
        $user    = Factory::getUser();
        $message = ['action'      => 'actionlogs', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user->id, 'title'       => $user->username, 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'userid'      => $user->id, 'username'    => $user->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id];
    ...
    public function onAfterPurge($group = 'all')
    {
        ...
        $message = ['action'      => 'cache', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user->id, 'title'       => $user->username, 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'userid'      => $user->id, 'username'    => $user->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'group'       => $group];
    ...
    public function onAfterDispatch()
    {
        ...
        $message = ['action'      => 'API', 'verb'        => $verb, 'username'    => $user->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'url'         => htmlspecialchars(urldecode((string) $this->getApplication()->get('uri.route')), ENT_QUOTES, 'UTF-8')];
    ...
    public function onJoomlaAfterUpdate($oldVersion = null)
    {
        ...
        $message = ['action'      => 'joomlaupdate', 'type'        => 'PLG_ACTIONLOG_JOOMLA_TYPE_USER', 'id'          => $user->id, 'title'       => $user->username, 'itemlink'    => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'userid'      => $user->id, 'username'    => $user->username, 'accountlink' => 'index.php?option=com_users&task=user.edit&id=' . $user->id, 'version'     => JVERSION, 'oldversion'  => $oldVersion];
