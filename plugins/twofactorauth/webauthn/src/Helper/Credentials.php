...
namespace Joomla\Plugin\Twofactorauth\Webauthn\Helper;
...
use Joomla\Plugin\Twofactorauth\Webauthn\CredentialRepository;
use Joomla\Plugin\Twofactorauth\Webauthn\Hotfix\Server;
...
abstract class Credentials
{
	...
	public static function requestAttestation(User $user): string
	{
		...
		$session->set('plg_twofactorauth_webauthn.publicKeyCredentialCreationOptions', base64_encode(serialize($publicKeyCredentialCreationOptions)));
		$session->set('plg_twofactorauth_webauthn.registration_user_id', $user->id);
	...
	public static function verifyAttestation(string $data): ?PublicKeyCredentialSource
	{
		...
		$encodedOptions = $session->get('plg_twofactorauth_webauthn.publicKeyCredentialCreationOptions', null);

		if (empty($encodedOptions))
		{
			throw new RuntimeException(Text::_('PLG_TWOFACTORAUTH_WEBAUTHN_ERR_CREATE_NO_PK'));
		}
		...
		$storedUserId = $session->get('plg_twofactorauth_webauthn.registration_user_id', 0);
		...
		{
			throw new RuntimeException(Text::_('PLG_TWOFACTORAUTH_WEBAUTHN_ERR_CREATE_INVALID_USER'));
		}
	...
	public static function requestAssertion(int $userId): string
	{
		...
		$session = Factory::getApplication()->getSession();
		$session->set('plg_twofactorauth_webauthn.publicKeyCredentialRequestOptions', base64_encode(serialize($publicKeyCredentialRequestOptions)));
		$session->set('plg_twofactorauth_webauthn.userHandle', $userId);
		$session->set('plg_twofactorauth_webauthn.userId', $userId);
	...
	public static function verifyAssertion(string $response): void
	{
		...
		$encodedPkOptions = $session->get('plg_twofactorauth_webauthn.publicKeyCredentialRequestOptions', null);
		$userHandle       = $session->get('plg_twofactorauth_webauthn.userHandle', null);
		$userId           = $session->get('plg_twofactorauth_webauthn.userId', null);

		$session->set('plg_twofactorauth_webauthn.publicKeyCredentialRequestOptions', null);
		$session->set('plg_twofactorauth_webauthn.userHandle', null);
		$session->set('plg_twofactorauth_webauthn.userId', null);
