...
class PlgSystemLanguageFilter extends JPlugin
{
	 ...
	 * Build rule.
	 * 
	 * @param   JRouter  &$router  A router object.
	 * @param   JUri     &$uri     A URI object.
	 * 
	 * @return  void
	 */
			...
			else
			{
				if ($app->isSite())
				{
					$app->setUserState(
						...
						'index.php?Itemid=' . $app->getMenu()->getDefault($lang_code)->id . '&lang=' . self::$lang_codes[$lang_code]->sef
					);
					self::$tag = $lang_code;
