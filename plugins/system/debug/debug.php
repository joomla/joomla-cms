    ...
    protected function displayProfileInformation() 
    {
        ...
        foreach (JProfiler::getInstance('Application')->getMarks() as $mark) 
        {
            ...
            $htmlMark = sprintf(
                    ...
                    . ' %s: %s', $mark->time, $mark->totalTime, $mark->memory, $mark->totalMemory, $mark->prefix, $mark->label
            );
        ...
        foreach ($log as $id => $query) 
        {
            ...
            if ($timings && isset($timings[$id * 2 + 1])) 
            {
                ...
                if (isset($callStacks[$id][0]['memory'])) 
                {
                    ...
                    $htmlTiming .= ' ' . '<span class="dbg-query-memory">' . JText::sprintf('PLG_DEBUG_MEMORY_USED_FOR_QUERY', sprintf('<span class="label ' . $labelClass . '">%.3f&nbsp;MB</span>', $memoryUsed / 1048576), sprintf('<span class="label">%.3f&nbsp;MB</span>', $memoryBeforeQuery / 1048576)
                            )
                            . '</span>';
