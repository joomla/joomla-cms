 ...
 * @since       2.5
 */
class plgFinderContactsInstallerScript
{
	/**
	...
	function activateButton()
	{
		...
		$query->where($db->quoteName('name').' = '.$db->quote('plg_finder_contacts'));
		$db->setQuery($query);
		if (!$db->query())
		{
			JError::raiseNotice(1, JText::_('PLG_FINDER_CONTACTS_ERROR_ACTIVATING_PLUGIN'));
		}
	}
}
