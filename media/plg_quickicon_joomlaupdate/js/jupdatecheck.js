			...
			if (updateInfoList instanceof Array) {
				if (updateInfoList.length < 1) {
					...
					if (updateInfo.version != plg_quickicon_jupdatecheck_jversion) {
						...
						var updateString = plg_quickicon_joomlaupdate_text.UPDATEFOUND2.replace("%s", updateInfo.version + "");
						jQuery('#system-message-container').before(
							'<button class="alert alert-error span12 center" onclick="document.location=\'' + plg_quickicon_joomlaupdate_url + '\'">'
								+ updateString + '</button>'
						);
					} else {
