...
(function () {
    var MediaManager = this.MediaManager = {
        ...
        initialize: function () {
            ...
            this.renamepath = document.id('renamepath');
            this.renameInput = document.id('renameInput');

            this.toolbarRename = $$('#toolbar-rename button');
        ...
        onloadframe: function () {
            ...
            this.frameurl = this.frame.location.href;
            this.toolbarRename[0].disabled = true;

            form = window.frames['folderframe'].document.id('mediamanager-form');
            checkboxes = form.getElements('input[type=checkbox]');
            checkboxes.addEvent('click', function () {
                count = form.getElements('input:checked').length;
                toolbarRename = $$('#toolbar-rename button');
                renameSubmit = document.id('renameSubmit');
                if (count == 1) {
                    toolbarRename[0].disabled = false;
                    renameSubmit.disabled = false;
                }
                else {
                    toolbarRename[0].disabled = true;
            ...
            if (folder) {
                ...
                this.renamepath.value = basepath + '/' + folder;
//			node = this.tree.get('node_'+folder);
                ...
                this.renamepath.value = basepath;
//			node = this.tree.root;
            }
            /*
             if (node) {
