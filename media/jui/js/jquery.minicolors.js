...
if(jQuery) (function($) {
	...
	function init(input, settings) {
		...
		input
			...
			*/
			.addClass('minicolors-input ' + $input_class)
			/*</JUI>*/
	...
	function updateFromControl(input, target) {
		...
		*/
		...
		/*</JUI>*/
			...
			/*<JUI>*/
			/* Original: in a separated target '.minicolors-opacity-slider' (before "Set swatch color")
			// Handle opacity
			...
			*/
			...
			if ( validate === 'color' ) {
				...
				if ( opacity == 0 && settings.keywords.indexOf('transparent') >= 0 ) {
					...
					value = 'transparent';
				} else if ( opacity && input.minicolors('rgbObject').a < 1 && rgb ) {
					// Set a rgba string
	...
	$(document)
		...
		.on('blur.minicolors', '.minicolors-input', function(event) {
			...
			*/
			var opacity = input.minicolors('rgbObject').a ? input.minicolors('rgbObject').a : '1',
				validate = input.attr('data-validate'),
