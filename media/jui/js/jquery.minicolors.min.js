...
case'create':default:if(method!=='create')data=method;$(this).each(function(){init($(this),data);});return $(this);}}});function init(input,settings){var minicolors=$('<span class="minicolors" />'),defaultSettings=$.minicolors.defaultSettings;if(input.data('minicolors-initialized'))return;if(jQuery(document.querySelector("html")).attr('dir')=='rtl')
{if(settings.position==='default')settings.position='left';}
settings=$.extend(true,{},defaultSettings,settings);if(jQuery(document.querySelector("html")).attr('dir')=='rtl')
{if(settings.swatchPosition==='left')settings.swatchPosition='right';}
minicolors.addClass('minicolors-theme-'+settings.theme).addClass('minicolors-swatch-position-'+settings.swatchPosition).toggleClass('minicolors-swatch-left',settings.swatchPosition==='left').toggleClass('minicolors-with-opacity',settings.opacity);if(settings.position!==undefined){$.each(settings.position.split(' '),function(){minicolors.addClass('minicolors-position-'+this);});}
var validate=input.attr('data-validate');if(validate==='color'){$input_class='hex';$input_size='7';$input_maxlength='7';}else{$input_class=settings.alpha?'rgba':'hex-keywords';$input_size=settings.alpha?'25':'11';$input_maxlength=settings.alpha?'25':'11';}
input.addClass('minicolors-input '+$input_class).data('minicolors-initialized',true).data('minicolors-settings',settings).prop('size',$input_size).prop('maxlength',$input_maxlength).wrap(minicolors).after('<span class="minicolors-panel minicolors-slider-'+settings.control+'">'+'<span class="minicolors-slider">'+'<span class="minicolors-picker"></span>'+'</span>'+'<span class="minicolors-opacity-slider">'+'<span class="minicolors-picker"></span>'+'</span>'+'<span class="minicolors-grid">'+'<span class="minicolors-grid-inner"></span>'+'<span class="minicolors-picker"><span></span></span>'+'</span>'+'</span>');input.parent().find('.minicolors-panel').on('selectstart',function(){return false;}).end();if(settings.swatchPosition==='left'){input.before('<span class="minicolors-swatch"><span></span></span>');}else{input.after('<span class="minicolors-swatch"><span></span></span>');}
...
if(settings.opacity)input.attr('data-opacity',opacity);var rgb=hex2rgb(hex),validate=input.attr('data-validate'),opacity=input.attr('data-opacity')===''?1:keepWithin(parseFloat(input.attr('data-opacity')).toFixed(2),0,1);if(isNaN(opacity))opacity=1;if(validate==='color'){value=convertCase(hex,settings.letterCase);}else{if(opacity==0&&settings.keywords.indexOf('transparent')>=0){value='transparent';}else if(opacity&&input.minicolors('rgbObject').a<1&&rgb){value='rgba('+rgb.r+', '+rgb.g+', '+rgb.b+', '+parseFloat(opacity)+')';}else{value=convertCase(hex,settings.letterCase);}}
input.val(value);}
swatch.find('SPAN').css({backgroundColor:hex,opacity:opacity});doChange(input,hex,opacity);}
function updateFromInput(input,preserveInputValue,firstRun){var hex,hsb,opacity,x,y,r,phi,minicolors=input.parent(),settings=input.data('minicolors-settings'),swatch=minicolors.find('.minicolors-swatch'),grid=minicolors.find('.minicolors-grid'),slider=minicolors.find('.minicolors-slider'),opacitySlider=minicolors.find('.minicolors-opacity-slider'),gridPicker=grid.find('[class$=-picker]'),sliderPicker=slider.find('[class$=-picker]'),opacityPicker=opacitySlider.find('[class$=-picker]');hex=convertCase(parseHex(input.val(),true),settings.letterCase);if(!hex)hex=convertCase(parseHex(settings.defaultValue,true));hsb=hex2hsb(hex);var rgb=hex2rgb(hex),validate=input.attr('data-validate'),opacity=input.attr('data-opacity')===''?1:keepWithin(parseFloat(input.attr('data-opacity')).toFixed(2),0,1);if(isNaN(opacity))opacity=1;if(validate==='color'){value=hex;}else{if(settings.keywords.indexOf(input.val())>=0){value=input.val();}else if(opacity&&input.minicolors('rgbObject').a<1&&rgb){value='rgba('+rgb.r+', '+rgb.g+', '+rgb.b+', '+parseFloat(opacity)+')';}else{value=hex;}}
if(!preserveInputValue){input.val(value);}
if(settings.opacity){opacity=input.attr('data-opacity')===''?1:keepWithin(parseFloat(input.attr('data-opacity')).toFixed(2),0,1);if(isNaN(opacity))opacity=1;input.attr('data-opacity',opacity);swatch.find('SPAN').css('opacity',opacity);y=keepWithin(opacitySlider.height()-(opacitySlider.height()*opacity),0,opacitySlider.height());opacityPicker.css('top',y+'px');}
if(input.val()==='transparent'){swatch.find('SPAN').css('opacity',0);}
...
return'#'+string;}
function isRgba(string){rgb=string.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i);return(rgb&&rgb.length===4)?true:'';}
