...
window.JoomlaCalendar = function (params) {
	...
	this.params.inputField =  params.inputField;
	 ...
	 */
	this.params.stringFD = 0;                                      // 0=>Sun, 1=>Mon etc
	this.params.stringTODAY = "Today";                       // Today
	this.params.stringWEEKEND = [0, 6];                  // 0,6
	this.params.stringWK = "wk";                                   // wk
	this.params.stringTIME = "Time:";                          // Time:
	this.params.stringTIMEAM = "AM";
	this.params.stringTIMEPM = "PM";
	this.params.stringDN = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
	this.params.stringSDN = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
	this.params.stringMN = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
	this.params.stringSMN = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
...
JoomlaCalendar.prototype.callHandler = function () {
	...
	var update = this.dateClicked;

	if (this.params.dateType == 'gregorian') {
		this.params.inputField.value = this.date.print(this.params.dateFormat, this.params.dateType);
		this.params.inputField.setAttribute('data-alt-value', this.date.print('%Y-%m-%d %H:%M:%S', this.params.dateType));
	} else {
		this.jGregorianDate = Date.convertFromLocal(date);
		this.params.inputField.value = this.jGregorianDate.print(this.params.dateFormat, this.params.dateType);
		this.params.inputField.setAttribute('data-alt-value', this.jGregorianDate.print('%Y-%m-%d %H:%M:%S', this.params.dateType));
	}

	if (typeof this.params.inputField.onchange == "function")
		this.params.inputField.onchange();

	if (update && typeof this.params.onUpdate == "function")
		this.params.onUpdate(this);

	if (update && this.dateClicked)
		this.closeHandler();
};

/** Calls the second user handler (closeHandler). */
JoomlaCalendar.prototype.closeHandler = function () {
	this.hide();
	this.destroy();
};
...
JoomlaCalendar.prototype.create = function (parent) {
	...
	console.log(parent);
	if (this.params.inputField.getAttribute('data-alt-value') != '0000-00-00 00:00:00') {
		if (this.params.dateType == 'gregorian') {
			this.date = Date.parseDate(this.params.inputField.getAttribute('data-alt-value'), '%Y-%m-%d %H:%M:%S', this.params.dateType);
		} else {

			this.date = Date.parseDate(this.params.inputField.getAttribute('data-alt-value'), '%Y-%m-%d %H:%M:%S', this.params.dateType);
			this.date = DateToLocal(tmpLocalDate);
		}
	} else {
		this.date = new Date();
		if (this.params.dateType != 'gregorian') this.date = DateToLocal(this.date);
	}
