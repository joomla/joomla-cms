  ...
  class JoomlaFieldColorSlider {
    ...
    setBackground(exceptSlider) {
      Array.prototype.forEach.call(this.sliders, slider => {
        ...
        let colors = [];
        let endValue = 100; // Longer start color so slider selection matches displayed colors
    ...
    setInitValue() {
      ...
      let hsl = []; // When given value is a number, use it as defined format and get rest from default value
    ...
    rgbToHex(rgb) {
      ...
      let b = rgb[2].toString(16).toUpperCase(); // Double value for hex with '#' and 6 chars
    ...
    hslToRgb([h, sat, light, alpha]) {
      let r = 1;
      let g = 1;
      let b = 1; // Saturation and light were calculated as 0.24 instead of 24%
