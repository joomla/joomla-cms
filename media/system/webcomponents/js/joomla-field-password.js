;(function(customElements) {
	...
	class JoomlaFieldPassword extends HTMLElement {
		...
		connectedCallback() {
			...
			if (this.minLength + this.minIntegers + this.minSymbols + this.minUppercase + this.minLowercase > 0) {
				let startClass = '';
				let initialVal = '';
				...
				this.input.addEventListener('keyup', this.setValue.bind(this));
				...
				if (document.formvalidator) {
					document.formvalidator.setHandler(this.getAttribute('validation-handler'), this.handler(value).bind(this));
				}
			}
		...
		getMeter() {
			...
			const score = strength.getScore(this.input.value),
			      ...
			      label = this.input.parentNode.parentNode.querySelector('#password-' + i);
