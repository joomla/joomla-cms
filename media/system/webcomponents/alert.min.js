...
var _createClass = function () {
  ...
  function b() {
    return _classCallCheck(this, b), _possibleConstructorReturn(this, (b.__proto__ || Object.getPrototypeOf(b)).call(this));
      ...
      this.removeEventListener('close.bs.alert', this), this.removeEventListener('closed.bs.alert', this);var c = this.querySelector('div');c&& c.firstChild && c.firstChild.removeEventListener('click', this);
      ...
      this.dispatchCustomEvent('close.bs.alert');var c = this.querySelector('div');('WebkitTransition' in document.documentElement.style || 'transition' in document.documentElement.style) && c.addEventListener('transitionend', function (d) {
        d.target.parentNode.dispatchCustomEvent('closed.bs.alert'), d.target.parentNode.removeChild(d.target);
      }, !1), c.classList.remove('show'), (!1 in document.documentElement.style || !1 in document.documentElement.style) && (this.dispatchCustomEvent('closed.bs.alert'), this.parentNode.removeChild(this));
      ...
      var c = this.querySelector('div.alert');if (!this.querySelector('button[aria-label="Close"]') && c) {
        var d = document.createElement('button');d.setAttribute('type', 'button'), d.setAttribute('aria-label', 'Close'), d.classList.add('close'), d.innerHTML = '<span aria-hidden="true">&times;</span>', c.firstChild ? c.insertBefore(d, c.firstChild) : c.appendChild(d), c.firstChild && c.firstChild.addEventListener('click', function (e) {
          var f = e.target;'button' === e.target.parentNode.tagName.toLowerCase() && (f = e.target.parentNode), f.parentNode.parentNode.close();
        });
      }
    } }, { key: 'removeCloseButton', value: function removeCloseButton() {
      var c = this.querySelector('div');c.firstChild && (c.firstChild.removeEventListener('click', this), c.removeChild(this.firstChild));
