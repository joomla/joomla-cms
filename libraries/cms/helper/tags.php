...
class JHelperTags
{
	 ...
	 *
	 * @return  array|boolean  If successful, array with new tag titles replaced by tag ids. Otherwise false.
	 *
	 * @since   3.1
	 */
	...
	public function postStoreProcess($table)
	{
		...
		if ($this->tagsChanged || $tags)
		{
			...
			else
			{
				...
				$result = $result && $this->tagItem($ucmId, $table, $tags, true);
			}
		}
	...
	{
		// Get the new tags
		$newTags = $this->createTagsFromMetadata($table->metadata);

		// Get rid of the tags key in the table's metadata field
		if ($table->metadata)
		{
			$metadataObject = json_decode($table->metadata);
			if (is_object($metadataObject) && property_exists($metadataObject, 'tags'))
			{
				unset($metadataObject->tags);
			}
		}
		else
		{
		...
		// If there are no new tags just
		if ($newTags == false)
		{
			$newTags = (isset($newMetaObject->tags)) ? $newMetaObject->tags : null;
		}

		$table->metadata = json_encode($metadataObject);

		// Get the old tags from the database
		$key = $table->getKeyName();

		// If this is a new record and there are no tags we change nothing
		if (empty($table->$key) && empty($newTags))
		{
