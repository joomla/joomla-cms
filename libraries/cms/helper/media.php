...
class JHelperMedia
{
	...
	public function canUpload($file, $component = 'com_media')
	{
		...
		if (empty($file['name']))
		{
			$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_UPLOAD_INPUT'), 'info');
		...
		{
			$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNFILENAME'), 'info');
		...
		if (count($filetypes) < 2)
		{
			...
			$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNFILETYPE'), 'info');
		...
		{
			$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNFILETOOLARGE'), 'info');
		...
		if ($params->get('restrict_uploads', 1))
		{
			...
			if (in_array($filetype, $images))
			{
				...
				if (!empty($file['tmp_name']))
				{
					...
					{
						$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNINVALID_IMG'), 'info');
			...
			elseif (!in_array($filetype, $ignored))
			{
				...
				if (function_exists('finfo_open') && $params->get('check_mime', 1))
				{
					...
					{
						$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNINVALID_MIME'), 'info');
				...
				{
					$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNNOTADMIN'), 'info');
		...
		foreach ($html_tags as $tag)
		{
			...
			{
				$app->enqueueMessage(JText::_('JLIB_MEDIA_ERROR_WARNIEXSS'), 'info');
