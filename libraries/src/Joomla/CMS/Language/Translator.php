...
class Translator
{
	...
	public function _($string, $jsSafe = false, $interpretBackSlashes = true, $script = false)
	{
		...
		if ($this->passSprintf($string, $jsSafe, $interpretBackSlashes, $script))
		{
			return $string;
		}
		...
		if ($script)
		{
			$this->strings[$string] = $this->language->_($string, $jsSafe, $interpretBackSlashes);

			return $string;
		}

		return $this->language->_($string, $jsSafe, $interpretBackSlashes);
	}
	...
	private function passSprintf(&$string, $jsSafe = false, $interpretBackSlashes = true, $script = false)
	{
		...
		{
			$string_parts[$i] = $this->language->_($str, $jsSafe, $interpretBackSlashes);
		}
		...
		if ($script)
		{
			foreach ($string_parts as $i => $str)
			{
				$this->strings[$str] = $string_parts[$i];
			}
		}
	 ...
	 *
	 * Examples:
	 * `<?php echo $translator->alt('JALL', 'language'); ?>` will generate a 'All' string in English but a "Toutes" string in French
	 * `<?php echo $translator->alt('JALL', 'module'); ?>` will generate a 'All' string in English but a "Tous" string in French
	 *
	 * @param   string   $string                The string to translate.
	...
	{
		if ($this->language->hasKey($string . '_' . $alt))
		{
			$string .= '_' . $alt;
		}
	...
	/**
	 * Like $translator->sprintf but tries to pluralise the string.
	 *
	 ...
	 *
	 * Examples:
	 * `<script>alert(Joomla.JText._('<?php echo $translator->plural("COM_PLUGINS_N_ITEMS_UNPUBLISHED", 1, array("script"=>true)); ?>'));</script>`
	 * will generate an alert message containing '1 plugin successfully disabled'
	 * `<?php echo $translator->plural('COM_PLUGINS_N_ITEMS_UNPUBLISHED', 1); ?>` will generate a '1 plugin successfully disabled' string
	 *
	...
	public function script($string = null, $jsSafe = false, $interpretBackSlashes = true)
	{
		...
		if ($string !== null)
		{
			...
			$this->strings[strtoupper($string)] = $this->language->_($string, $jsSafe, $interpretBackSlashes);
