    ...
    public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
    {
        ...
        foreach ($arr as $elementKey => &$element) {
            ...
            if (\is_array($element)) {
                ...
                if ($options['option.attr'] && isset($element[$options['option.attr']])) {
                    $attr = $element[$options['option.attr']];
                }

                if ($options['option.id'] && isset($element[$options['option.id']])) {
                    $id = $element[$options['option.id']];
                }

                if ($options['option.label'] && isset($element[$options['option.label']])) {
                    $label = $element[$options['option.label']];
                }

                if ($options['option.disable'] && isset($element[$options['option.disable']]) && $element[$options['option.disable']]) {
                ...
                if ($options['option.class'] && isset($element->{$options['option.class']}) && $element->{$options['option.class']}) {
                    $extra .= ' class="' . $element->{$options['option.class']} . '"';
                }

                if ($options['option.onclick'] && isset($element->{$options['option.onclick']}) && $element->{$options['option.onclick']}) {
