...
abstract class Tag
{
    ...
    public static function ajaxfield($selector = '#jform_tags', $allowCustom = true)
    {
        ...
        Factory::getDocument()->addScriptOptions(
            'ajax-chosen',
            ['url'            => Uri::root() . 'index.php?option=com_tags&task=tags.searchAjax', 'debug'          => JDEBUG, 'selector'       => $selector, 'type'           => 'GET', 'dataType'       => 'json', 'jsonTermKey'    => 'like', 'afterTypeDelay' => 500, 'minTermLength'  => $minTermLength]
        );
        ...
        if ($allowCustom) {
            ...
            Factory::getDocument()->addScriptOptions(
                'field-tag-custom',
                ['minTermLength' => $minTermLength, 'selector'      => $selector, 'allowCustom'   => Factory::getUser()->authorise('core.create', 'com_tags') ? $allowCustom : false]
            );
        }
    }
}
