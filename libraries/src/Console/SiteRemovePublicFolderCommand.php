...
/**
 * Console command for removing a public folder
 *
 * @since  __DEPLOY_VERSION__
 */
class SiteRemovePublicFolderCommand extends AbstractCommand
{
    /**
     ...
     * @since  __DEPLOY_VERSION__
     */
    protected static $defaultName = 'site:remove-public-folder';
    ...
    protected function doExecute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->ioStyle->title('Remove a public folder');
        ...
        if (!is_dir($this->publicFolder)) {
            $this->ioStyle->fail("There is no folder for the path you provided. \nCheck your configuration.");
        ...
        if (!($generatorClass->cleanPublicFolder($this->publicFolder, false))) {
            return Command::FAILURE;
        }

        if (!($generatorClass->removePublicFolder($this->publicFolder))) {
            return Command::FAILURE;
        }

        $this->ioStyle->success("Public folder removed! \nAdjust your server configuration to serve from the root folder of Joomla!");
    ...
    {
        $help = "<info>%command.name%</info> will remove an existing public folder
		\nUsage: <info>php %command.full_name%</info>";

        $this->addOption('public-folder', null, InputOption::VALUE_REQUIRED, 'public folder absolute path');
        $this->setDescription('Remove a public folder');
        $this->setHelp($help);
    }
}
