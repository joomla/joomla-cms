...
abstract class HtmlHelper
{
	 ...
	 */
	protected static $includePrefixes = array('\\Joomla\\CMS\\Html\\');
	...
	public static function _($key)
	{
		...
		foreach (self::$includePrefixes as $classPrefix)
		{
			// Compile the classname
			$class = $classPrefix . ucfirst($file);
	...
	/**
	 * Add a class prefix where HtmlHelper should search for helpers. You may
	 * either pass a string or an array of prefixes.
	 * The prefixes can be a class prefix or a namespace.
	 *
	 * @param   string  $classPathPrefix  A class prefix to search.
	 *
	 * @return  array  An array with directory elements
	 *
	 * @since   1.5
	 */
	public static function addIncludePrefixes($classPathPrefix = '')
	{
		// Force prefixes to array
		settype($classPathPrefix, 'array');

		// Loop through the class prefixes
		foreach ($classPathPrefix as $prefix)
		{
			...
			if (in_array($prefix, static::$includePrefixes))
			{
				continue;
			}

			// Append the prefix to the beginning of the array
			array_unshift(static::$includePrefixes, $prefix);
		}

		return static::$includePrefixes;
	}
