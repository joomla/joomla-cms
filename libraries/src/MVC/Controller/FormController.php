...
class FormController extends BaseController implements FormFactoryAwareInterface
{
	...
	public function save($key = null, $urlVar = null)
	{
		...
		$langKey = $this->text_prefix . ($recordId === 0 && $app->isClient(SiteApplication::CLIENT) ? '_SUBMIT' : '') . '_SAVE_SUCCESS';
		$prefix  = Factory::getLanguage()->hasKey($langKey) ? $this->text_prefix : 'JLIB_APPLICATION';

		$this->setMessage(Text::_($prefix . ($recordId === 0 && $app->isClient(SiteApplication::CLIENT) ? '_SUBMIT' : '') . '_SAVE_SUCCESS'));
