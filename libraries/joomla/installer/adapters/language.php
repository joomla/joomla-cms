...
class JInstallerLanguage extends JAdapterInstance
{
	...
	protected function _install($cname, $basePath, $clientId, &$element)
	{
		...
		if (!$this->_core)
		{
			...
			{
				$this->parent->abort(
					JText::sprintf('JLIB_INSTALLER_ABORT', JText::sprintf('JLIB_INSTALLER_ERROR_NO_CORE_LANGUAGE', $this->get('tag'))));
				return false;
			}
		}
		...
		else
		{
			...
			if ($this->parent->getUpgrade() ||
				 ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'update')) || is_a($updateElement, 'JXMLElement'))
				{
