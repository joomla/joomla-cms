...
class JInstallerModule extends JAdapterInstance
{
	...
	public function install()
	{
		...
		if ($cname = (string) $this->manifest->attributes()->client)
		{
			...
			{
				$this->parent->abort(
					JText::sprintf('JLIB_INSTALLER_ABORT_MOD_UNKNOWN_CLIENT', JText::_('JLIB_INSTALLER_' . $this->route), $client->name));
				return false;
			}
		...
		catch (JException $e)
		{
			...
			$this->parent->abort(
				JText::sprintf('JLIB_INSTALLER_ABORT_MOD_ROLLBACK', JText::_('JLIB_INSTALLER_' . $this->route), $db->stderr(true)));
		...
		if (file_exists($this->parent->getPath('extension_root')) && (!$this->parent->getOverwrite() || $this->parent->getUpgrade()))
		{
			...
			if ($this->parent->getUpgrade() ||
				...
				else if (!$this->parent->getOverwrite())
				{
					...
					$this->parent->abort(
						JText::sprintf('JLIB_INSTALLER_ABORT_MOD_INSTALL_DIRECTORY', JText::_('JLIB_INSTALLER_' . $this->route),
