	...
	public function install()
	{
		 ...
		 */
			...
			if ($installFile)
			{
				...
				if (!file_exists($this->parent->getPath('extension_administrator') . '/' . $installFile) ||
				...
				{
					// Make sure it hasn't already been copied (this would be an error in the XML install file)
					if (!file_exists($this->parent->getPath('extension_administrator') . '/' . $uninstallFile) ||
						 $this->parent->getOverwrite())
						{
		 ...
		 */
					...
					if (isset($this->manifest->install->sql))
					{
						...
						if ($utfresult === false)
						{
							// Install failed, rollback changes
							$this->parent->abort(
								JText::sprintf('JLIB_INSTALLER_ABORT_COMP_INSTALL_SQL_ERROR', $db->stderr(true)));
					...
					if ($this->get('install_script'))
					{
						...
						if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'install'))
						{
							if ($this->parent->manifestClass->install($this) === false)
							{
								// Install failed, rollback changes
								$this->parent->abort(
									JText::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_CUSTOM_INSTALL_FAILURE'));
						...
						if (!$row->store())
						{
							...
							$this->parent->abort(
								JText::sprintf('JLIB_INSTALLER_ABORT_COMP_INSTALL_ROLLBACK', $db->stderr(true)));
							return false;
						}
						...
						$uid = $update->find(
							array('element' => $this->get('element'), 'type' => 'component', 'client_id' => '', 'folder' => ''));
		...
		//$this->parent->abort(JText::sprintf('JLIB_INSTALLER_ABORT_COMP_INSTALL_ROLLBACK', $db->stderr(true)));
					...
					public function update()
					{
						...
						$this->parent->setPath('extension_site',
							JPath::clean(JPATH_SITE . '/components/' . $this->get('element')));
						$this->parent->setPath('extension_administrator', JPath::clean(JPATH_ADMINISTRATOR . '/components/' . $this->get('element')));
		...
		// Note: if we don't find the class, don't bother to copy the file
						...
						if (!file_exists($this->parent->getPath('extension_site')))
						{
							...
							{
								JError::raiseWarning(1,
									JText::sprintf('JLIB_INSTALLER_ERROR_COMP_UPDATE_FAILED_TO_CREATE_DIRECTORY_SITE',
		 ...
		 */
						...
						if (!file_exists($this->parent->getPath('extension_administrator')))
						{
							...
							{
								JError::raiseWarning(1,
									JText::sprintf('JLIB_INSTALLER_ERROR_COMP_UPDATE_FAILED_TO_CREATE_DIRECTORY_ADMIN',
										$this->parent->getPath('extension_administrator')));
								// Install failed, rollback any changes
								$this->parent->abort();
		 ...
		 */
						...
						if ($installFile)
						{
							...
							if (!file_exists($this->parent->getPath('extension_administrator') . '/' . $installFile) ||
								 $this->parent->getOverwrite())
								{
									...
									if (!$this->parent->copyFiles(array($path)))
									{
										// Install failed, rollback changes
										$this->parent->abort(
											JText::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_INSTALL'));
										return false;
									}
								}
							...
							if ($uninstallFile)
							{
								// Make sure it hasn't already been copied (this would be an error in the XML install file)
								if (!file_exists(
									$this->parent->getPath('extension_administrator') . '/' . $uninstallFile) || $this->parent->getOverwrite())
								{
									$path['src'] = $this->parent->getPath('source') . '/' . $uninstallFile;
									$path['dest'] = $this->parent->getPath('extension_administrator') . '/' . $uninstallFile;
									...
									if (!$this->parent->copyFiles(array($path)))
									{
										// Install failed, rollback changes
										$this->parent->abort(
											JText::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_UNINSTALL'));
							...
							if ($this->get('manifest_script'))
							{
								...
								if (!file_exists($path['dest']) || $this->parent->getOverwrite())
								{
									if (!$this->parent->copyFiles(array($path)))
									{
										// Install failed, rollback changes
										$this->parent->abort(
											JText::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_MANIFEST'));
		 ...
		 */
							...
							if ($this->manifest->update)
							{
								...
								if ($result === false)
								{
									// Install failed, rollback changes
									$this->parent->abort(
										JText::sprintf('JLIB_INSTALLER_ABORT_COMP_UPDATE_SQL_ERROR', $db->stderr(true)));
		 ...
		 */
								...
								if (!$row->store())
								{
									// Install failed, roll back changes
									$this->parent->abort(
										JText::sprintf('JLIB_INSTALLER_ABORT_COMP_UPDATE_ROLLBACK', $db->stderr(true)));
							...
							public function uninstall($id)
							{
								...
								$this->parent->setPath('extension_administrator',
									JPath::clean(JPATH_ADMINISTRATOR . '/components/' . $row->element));
								...
								$name = strtolower(
									JFilterInput::getInstance()->clean((string) $this->manifest->name, 'cmd'));
								if (substr($name, 0, 4) == "com_")
								{
									$element = $name;
								}
		...
		// Note: if we don't find the class, don't bother to copy the file
								...
								if ($this->parent->manifestClass &&
									...
									if ($uninstallFile)
									{
										...
										if (is_file(
											$this->parent->getPath('extension_administrator') . '/' . $uninstallFile))
										{
											ob_start();
											ob_implicit_flush(false);
		 ...
		 */
									...
									if (isset($this->manifest->uninstall->sql))
									{
										...
										if ($utfresult === false)
										{
											// Install failed, rollback changes
											JError::raiseWarning(100,
												JText::sprintf('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_SQL_ERROR', $db->stderr(true)));
											$retval = false;
										}
									}
									...
									$update = JTable::getInstance('update');
									$uid = $update->find(
										array('element' => $row->element, 'type' => 'component', 'client_id' => '', 'folder' => ''));
		 ...
		 */
									...
									foreach ($this->manifest->administration->submenu->menu as $child)
									{
										...
										if (!$table->setLocation($parent_id, 'last-child') || !$table->bind($data) || !$table->check() ||
											 !$table->store())
											{
												// Install failed, rollback changes
												return false;
			 ...
			 */
									...
									public function discover()
									{
										...
										foreach ($admin_components as $component)
										{
											...
											{
												$manifest_details = JApplicationHelper::parseXMLInstallFile(
													JPATH_ADMINISTRATOR . '/components/' . $component . '/' . str_replace('com_', '', $component) .
														 '.xml');
												$extension = JTable::getInstance('extension');
												$extension->set('type', 'component');
									...
									public function discover_install()
									{
										...
										$manifestPath = $client->path . '/components/' . $this->parent->extension->element . '/' . $short_element .
											 '.xml';
											...
											$this->parent->setPath('extension_site',
												JPath::clean(JPATH_SITE . '/components/' . $this->get('element')));
											$this->parent->setPath('extension_administrator',
												JPath::clean(JPATH_ADMINISTRATOR . '/components/' . $this->get('element')));
											$this->parent->setPath('extension_root', $this->parent->getPath('extension_administrator')); // copy this as its used as a common base
											...
											if ($manifestScript)
											{
												...
												{
													// create a new instance
													$this->parent->manifestClass = new $classname(
														$this);
													// and set this so we can copy it later
													$this->set('manifest_script', $manifestScript);
		 ...
		 */
											...
											// Time to build the admin menus
											if (!$this->_buildAdminMenus(
												$this->parent->extension->extension_id))
											{
												JError::raiseWarning(100, JText::_('JLIB_INSTALLER_ABORT_COMP_BUILDADMINMENUS_FAILED'));
		 ...
		 */
											...
											if ($this->get('install_script'))
											{
												...
												if (is_file($this->parent->getPath('extension_administrator') . '/' . $this->get('install_script')))
												{
													...
													require_once $this->parent->getPath('extension_administrator') . '/' . $this->get(
														'install_script');
