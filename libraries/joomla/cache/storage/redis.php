...
class JCacheStorageRedis extends JCacheStorage
{
	...
	protected function getConnection()
	{
		...
		if($this->_persistent){
			$redistest = self::$_redis->pconnect($server['host'], $server['port']);
		}else{
			$redistest = self::$_redis->connect($server['host'], $server['port']);
		}

		if ($redistest == false){
			throw new RuntimeException('Could not connect to redis server', 404);
		}
