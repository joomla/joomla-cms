...
class JOAuth1CredentialsStateTemporary extends JOAuth1CredentialsState
{
	...
	public function authorise($resourceOwnerId, $lifetime = 0)
	{
		...
		$this->properties['resource_owner_id'] = (int) $resourceOwnerId;
		$this->properties['verifier_key'] = $this->randomKey();
		$this->properties['type'] = JOAuth1Credentials::AUTHORISED;
		$this->properties['expiration_date'] = $lifetime ? time() + $lifetime : 0;

		// Persist the object in the database.
		$this->update();

		return new JOAuth1CredentialsStateAuthorised($this->db, $this->properties);
	}
	...
	public function deny()
	{
		...
		return new JOAuth1CredentialsStateDenied($this->db);
	}
