...
/**
 * OAuth Controller class for revoking credentials for the Joomla Platform.
 *
 ...
 * @since       12.3
 */
class JOAuth1ControllerRevoke extends JControllerBase
{
	/**
	...
	public function execute()
	{
		...
		if ($credentials->getType() !== JOAuth1Credentials::TOKEN)
		{
			...
			$this->app->setBody('The token is not for an access credentials set.');
		...
		// Make sure the user owns the credentials he or she is trying to revoke.
		if ($credentials->getResourceOwnerId() != $this->app->getIdentity()->get('id'))
		{
			$this->app->setHeader('status', '400');
			$this->app->setBody('You are not authorised to revoke those credentials.');
		...
		$this->app->setBody('Credentials revoked.');
	}
}
