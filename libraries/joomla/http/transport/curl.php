...
class JHttpTransportCurl implements JHttpTransport
{
    ...
    public function request($method, JUri $uri, $data = null, array $headers = null, $timeout = null, $userAgent = null)
    {
        ...
        // Allow for SSL verification to be toggled off
        if ($this->options->exists('curl.ssl.verifypeer') && !(bool)$this->options->get('curl.ssl.verifypeer')) {
            $options[CURLOPT_SSL_VERIFYPEER] = 0;
        }

        if ($this->options->exists('curl.ssl.verifyhost') && !(bool)$this->options->get('curl.ssl.verifyhost')) {
