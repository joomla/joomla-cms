	...
	function getInstance($uri = 'SERVER') {
		...
		if (!isset ($instances[$uri])) {
			...
			if ($uri == 'SERVER') {
				 ...
				 * to determine if we are running on apache or IIS.  If PHP_SELF or REQUEST_URI
				 * are present, we will assume we are running on apache.
				 */
				if (!empty ($_SERVER['PHP_SELF']) && !empty ($_SERVER['REQUEST_URI'])) {
	
					/*
					 * Lets check to see if we are running php from cgi on apache.  If so, we will
					 * get the URI from the REQUEST_URI server environment variable.  Otherwise we 
					 * will use the PHP_SELF environment variable.
					 */
					if (strpos(php_sapi_name(), 'cgi') !== false) {
						$theURI = $_SERVER['REQUEST_URI'];
					} else {
						$theURI = $_SERVER['PHP_SElF'];
					}
				/*
				 * Since we do not have either PHP_SELF or REQUEST_URI to work with, we will assume we are 
				 * running on IIS and will therefore need to work some magic with the SCRIPT_NAME and
				 * QUERY_STRING environment variables.
				 */
				} else {
					$theURI = $_SERVER['SCRIPT_NAME'].@ $_SERVER['QUERY_STRING'];
				}
			} else {
				/*
				 * We were given a URI
