...
JLoader::register('JDatabaseQueryMSSql', dirname(__FILE__) . '/mssqlquery.php');
...
class JDatabaseMSSql extends JDatabase
{
	...
	protected function __construct($options)
	{
		...
		*/
		...
		// Make sure the MSSQL extension for PHP is installed and enabled.
		if (!function_exists('mssql_connect'))
		{
			...
			{
				$this->errorNum = 1;
				$this->errorMsg = JText::_('JLIB_DATABASE_ERROR_ADAPTER_MSSQL');
				return;
			}
			else
			{
				throw new JDatabaseException(JText::_('JLIB_DATABASE_ERROR_ADAPTER_MSSQL'));
			}
		}

		// Attempt to connect to the server.
		if (!($this->connection = @ mssql_connect ($options['host'], $options['user'], $options['password'])))
		{
			...
			{
				$this->errorNum = 2;
				$this->errorMsg = JText::_('JLIB_DATABASE_ERROR_CONNECT_MSSQL');
				return;
			}
			else
			{
				throw new JDatabaseException(JText::_('JLIB_DATABASE_ERROR_CONNECT_MSSQL'));
			}
		}

		// Make sure that DB warnings are not returned as errors.
		//TODO: sqlsrv_configure('WarningsReturnAsErrors', 0);
	 ...
	 *
	 * @return  JDatabaseMSSql  Returns this object to support chaining.
	 *
	 * @since   11.1
	...
	{
		return mssql_rows_affected($this->cursor);
	}
	...
	{
		return mssql_num_rows($cursor ? $cursor : $this->cursor);
	}
	...
	public function getQuery($new = false)
	{
		if ($new)
		{
			...
			if (!class_exists('JDatabaseQueryMSSql'))
			{
				throw new JDatabaseException(JText::_('JLIB_DATABASE_ERROR_MISSING_QUERY'));
			}
			return new JDatabaseQueryMSSql($this);
		}
		else
		{
			return $this->sql;
	...
	*/
	...
	{
		$cursorVersion = mssql_query('SELECT @@VERSION', $this->connection );
		$version = mssql_fetch_array($cursorVersion);
		mssql_free_result($cursorVersion);
	...
	public function execute()
	{
		...
		// Execute the query.
		$this->cursor =  mssql_query($sql, $this->connection);
		...
		if (!$this->cursor)
		{
			...
			// Populate the errors.
			$this->errorNum = 88888; //Just for not left 0, could be changed
			$this->errorMsg = @ mssql_get_last_message() . 'SQL=' . $sql;
	...
	{
		return  mssql_fetch_row($cursor ? $cursor : $this->cursor); //Returns only a numeric array
	}
	...
	{
		return mssql_fetch_assoc ($cursor ? $cursor : $this->cursor);
	}
	...
	{
		return mssql_fetch_object($cursor ? $cursor : $this->cursor);
	}
	...
	{
		mssql_free_result($cursor ? $cursor : $this->cursor);
	}
	...
	public function explain()
	{
		...
		// SET SHOWPLAN_ALL ON - will make mssql to show some explain of query instead of run it
	...
	public function queryBatch($abortOnError = true, $transactionSafe = false)
	{
		...
		foreach ($queries as $query)
		{
			...
			if ($query != '')
			{
				$this->cursor = mssql_query($query, $this->connection);
				if ($this->_debug)
				{
					$this->count++;
					$this->log[] = $query;
