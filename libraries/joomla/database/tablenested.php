...
class JTableNested extends JTable
{
	...
	public function getPath($pk = null, $diagnostic = false)
	{
		...
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_GET_PATH_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	public function getTree($pk = null, $diagnostic = false)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_GET_TREE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	public function setLocation($referenceId, $position = 'after')
	{
		...
		if (($position != 'before') && ($position != 'after') &&
			...
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_INVALID_LOCATION', get_class($this)));
			$this->setError($e);
			return false;
		}
	...
	public function move($referenceId, $position = 'after', $pk = null)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_MOVE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
		if ($this->_debug) {
		...
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_INVALID_NODE_RECURSION', get_class($this)));
			$this->setError($e);
			return false;
		}
	...
	public function delete($pk = null, $children = true)
	{
		...
		if ($children)
		{
			...
			if (!$this->_db->query())
			{
				$e = new JException(JText::sprintf('JTABLENESTED_ERROR_DELETE_FAILED', get_class($this), $this->_db->getErrorMsg()));
				$this->setError($e);
				$this->_unlock();
				return false;
			}
	...
	public function check()
	{
		...
		if ($this->parent_id > 0)
		{
			...
			else
			{
				if ($this->_db->getErrorNum())
				{
					$e = new JException(JText::sprintf('JTABLENESTED_ERROR_CHECK_FAILED', get_class($this), $this->_db->getErrorMsg()));
					$this->setError($e);
				}
				else
				{
					$e = new JException(JText::sprintf('JTABLENESTED_ERROR_INVALID_PARENT_ID', get_class($this)));
					$this->setError($e);
				}
			}
		}
		...
		if (empty($this->$k))
		{
			...
			if ($this->_location_id >= 0)
			{
				...
				if ($this->_location_id == 0)
				{
					...
					if ($this->_db->getErrorNum())
					{
						$e = new JException(JText::sprintf('JTABLENESTED_ERROR_STORE_FAILED', get_class($this), $this->_db->getErrorMsg()));
						$this->setError($e);
						$this->_unlock();
						return false;
					}
			...
			else
			{
				...
				$e = new JException(JText::_('JTABLENESTED_ERROR_INVALID_PARENT_ID'));
				$this->setError($e);
				return false;
			}
		}
	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		if (empty($pks))
		{
			...
			{
				$e = new JException(JText::sprintf('JTABLENESTED_ERROR_NO_ROWS_SELECTED', get_class($this)));
				$this->setError($e);
				return false;
			}
		}
		...
		foreach ($pks as $pk)
		{
			...
			if ($checkoutSupport)
			{
				...
				if ($this->_db->loadResult())
				{
					$e = new JException(JText::sprintf('JTABLENESTED_ERROR_CHILD_ROWS_CHECKED_OUT', get_class($this)));
					$this->setError($e);
					return false;
				}
			}
			...
			if ($node->parent_id)
			{
				...
				if ($this->_db->getErrorNum())
				{
					$e = new JException(JText::sprintf('JTABLENESTED_ERROR_PUBLISH_FAILED', get_class($this), $this->_db->getErrorMsg()));
					$this->setError($e);
					return false;
				}
				...
				if (!empty($rows))
				{
					$e = new JException(JText::sprintf('JTABLENESTED_ERROR_ANCESTOR_NODES_LOWER_PUBLISHED_STATE', get_class($this)));
					$this->setError($e);
					return false;
				}
			}
	...
	public function orderUp($pk)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_ORDERUP_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			$this->_unlock();
			return false;
		}
	...
	public function orderDown($pk)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_ORDERDOWN_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			$this->_unlock();
			return false;
		}
	...
	public function getRootId()
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_GETROOTID_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
		...
		else
		{
			...
			elseif (property_exists($this, 'alias'))
			{
				...
				{
					$e = new JException(JText::_('JTABLENESTED_ERROR_ROOT_NODE_NOT_FOUND'));
					$this->setError($e);
					return false;
				}
			}
	...
	public function rebuild($parentId = null, $leftId = 0, $level = 0, $path = '')
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_REBUILD_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	public function rebuildPath($pk = null)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_REBUILDPATH_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	protected function _getNode($id, $key = null)
	{
		...
		{
			$e = new JException(JText::sprintf('JTABLENESTED_ERROR_GETNODE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
