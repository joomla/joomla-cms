...
class JFilterInput extends InputFilter
{
	...
	protected function cleanTags($source)
	{
		...
		while ($offset < $length)
		{
			...
			// Now we have something like 'span class="" style=""', '/span', 'hr/' or 'hr /' or 'hr disabled /'
			$tagContent = substr($source, $offset + 1, $tagOpenEndOffset - 1 - $offset);

			// All whitespaces replace by 0x20
			$tagNormalized = preg_replace('/\s/', ' ', $tagContent);
			$tagLength     = strlen($tagContent);
			$spaceOffset   = strpos($tagNormalized, ' ');

			// Are we an open tag or a close tag?
			$isCloseTag = $tagContent[0] == '/' ? 1 : 0;

			if ($spaceOffset !== false)
			{
				$tagName = substr($tagContent, $isCloseTag, $spaceOffset - $isCloseTag);
			}
			else
			{
				$tagName = substr($tagContent, $isCloseTag, $tagLength - $isCloseTag);
			}
