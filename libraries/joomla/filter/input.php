...
class JFilterInput extends JObject
{
	...
	public function clean($source, $type = 'string')
	{
		...
		switch (strtoupper($type))
		{
			...
			case 'WORD':
                                $result = (string) preg_replace_callback('/[^A-Z_]/i', function(){ return '';}, $source);
				break;

			case 'ALNUM':
				$result = (string) preg_replace_callback('/[^A-Z0-9]/i', function(){ return '';}, $source);
				break;

			case 'CMD':
				$result = (string) preg_replace_callback('/[^A-Z0-9_\.-]/i', function(){ return '';}, $source);
				$result = ltrim($result, '.');
				break;
			...
			case 'USERNAME':
				$result = (string) preg_replace_callback('/[\x00-\x1F\x7F<>"\'%&]/', function(){ return '';}, $source);
				break;
	...
	protected function _decode($source)
	{
		...
		// Convert decimal
                $source = @preg_replace_callback('/&#(\d+);/me', function(){ "utf8_encode(chr(\\1))";}, $source); // decimal notation
		// Convert hex
                $source = @preg_replace_callback('/&#x([a-f0-9]+);/mei', function(){ return "utf8_encode(chr(0x\\1))";}, $source); // hex notation
		return $source;
	}
	...
	protected function _stripCSSExpressions($source)
	{
		// Strip any comments out (in the form of /*...*/)
                $test = preg_replace_callback('#\/\*.*\*\/#U', function(){ return '';}, $source);
		// Test for :expression
