...
class JBrowser extends JObject
{
	...
	public function match($userAgent = null, $accept = null)
	{
		...
		if (!empty($this->_agent))
		{
			...
			elseif (strpos($this->_lowerAgent, 'elaine/') !== false || strpos($this->_lowerAgent, 'palmsource') !== false ||
				 ...
				 */
					...
					elseif (preg_match('|Konqueror/([0-9]+)|', $this->_agent, $version) ||
						...
						elseif (strpos($this->_agent, 'UP/') !== false || strpos($this->_agent, 'UP.B') !== false ||
					...
					public function isViewable($mimetype)
					{
						...
						if (!empty($this->_accept))
						{
							...
							if ($this->isBrowser('mozilla') && ($mimetype == 'image/pjpeg') &&
