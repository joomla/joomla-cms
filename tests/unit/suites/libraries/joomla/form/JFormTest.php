...
class JFormTest extends TestCaseDatabase
{
	...
	/**
	 * Test for JForm::addFieldOption method.
	 *
	 * @return void
	 */
	public function testAddFieldOption()
	{
		...
		$this->assertThat(
			$form->load('<form><field type="list" name="test_list"></field></form>'),
			$this->isTrue(),
			'Line:' . __LINE__ . ' XML string should load successfully.'
		);
		...
		$this->assertThat(
			$form->addFieldOption('test_list', '2', 'Unsure'),
			$this->isTrue(),
			'Line:' . __LINE__ . ' The method should return true.'
		);

		$this->assertThat(
			$form->getFieldXml('test_list')->asXML(),
			$this->stringContains('<option value="2">Unsure</option>'),
			'Line:' . __LINE__ . ' The method should have added the new option [2 => Unsure].'
		);

		// Test adding an option with attributes
		$this->assertThat(
			$form->addFieldOption('test_list', '3', 'Certainly', null, null, array('class' => 'disabled', 'selected' => 'selected')),
			$this->isTrue(),
			'Line:' . __LINE__ . ' The method should return true.'
		);
		...
		$this->assertThat(
			$form->getFieldXml('test_list')->asXML(),
			$this->stringContains('<option value="3" class="disabled" selected="selected">Certainly</option>'),
			'Line:' . __LINE__ . ' The method should have added the new option [3 => Certainly] and the two option attributes.'
		);

		// Test adding an option inside an option group
		$form->addFieldOption('test_list', '4', 'Whatever', 'Group');

		$this->assertThat(
			$form->getFieldXml('test_list')->asXML(),
			$this->stringContains('<group label="Group"><option value="4">Whatever</option></group>'),
			'Line:' . __LINE__ . ' The method should have added the new option [4 => Whatever] in an option group "Group".'
		);

		// Test adding an option inside an existing option group
		$form->addFieldOption('test_list', '5', 'Another', 'Group');

		$this->assertThat(
			$form->getFieldXml('test_list')->asXML(),
			$this->stringContains('<group label="Group"><option value="4">Whatever</option><option value="5">Another</option></group>'),
			'Line:' . __LINE__ . ' The method should have added the new option [5 => Another] in the existing option group "Group".'
		);

		// Test adding an option inside a new option group
		$form->addFieldOption('test_list', '6', 'More', 'Group 2');

		$this->assertThat(
			$form->getFieldXml('test_list')->asXML(),
			$this->stringContains('<group label="Group 2"><option value="6">More</option></group>'),
			'Line:' . __LINE__ . ' The method should have added the new option [6 => More] in a new option group "Group 2".'
		);
	}
