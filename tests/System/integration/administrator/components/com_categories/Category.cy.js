describe('Test in backend that the category form', () => {
  beforeEach(() => {
    ...
    // Clear the filter
    cy.visit(
      '/administrator/index.php?option=com_categories&extension=com_content&filter=',
    );
  });
  afterEach(() =>
    cy.task(
      'queryDB',
      "DELETE FROM #__categories WHERE title = 'Test category'",
    ),
  );
  ...
  it('can change access level of a test category', () => {
    cy.db_createCategory({ title: 'Test category' }).then((id) => {
      cy.visit(
        `/administrator/index.php?option=com_categories&task=category.edit&id=${id}&extension=com_content`,
      );
  ...
  it('check redirection to list view', () => {
    cy.visit(
      '/administrator/index.php?option=com_categories&task=category.add&extension=com_content&filter[published]=',
    );
    cy.intercept(
