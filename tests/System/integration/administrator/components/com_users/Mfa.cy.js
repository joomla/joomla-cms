...
describe('Test in backend that the user', () => {
  afterEach(() =>
    cy
      .db_getUserId()
      .then((uid) =>
        cy.task('queryDB', `DELETE FROM #__user_mfa WHERE user_id = ${uid}`),
      ),
  );
  ...
  it('can login with Multi-factor Authentication (email)', () => {
    ...
    cy.get(
      '#myTab div[role="tablist"] button[aria-controls="multifactorauth"]',
    ).click();
    cy.task('clearEmails');
    cy.get(
      '.com-users-methods-list-method-name-email a.com-users-methods-list-method-addnew',
    ).click();
    ...
    cy.task('getMails').then((mails) => {
      ...
      cy.wrap(/code is -(\d{6})-$/.exec(mails[0].headers.subject)[1])
        .as('code')
        .then((code) =>
          cy
            .wrap(mails[0].body)
            .should('have.string', `Your authentication code is ${code}.`),
        );
      cy.wrap(mails[0].html).should('be.false');
    });
    cy.get('@code').then((code) =>
      cy.get('#com-users-method-code').clear().type(code),
    );
    cy.get('#com-users-method-edit').submit();
    cy.get(
      '.com-users-methods-list-method-name-email .com-users-methods-list-method-record',
    ).contains('Test Code');
    ...
    cy.task('getMails').then((mails) => {
      ...
      cy.wrap(/code is -(\d{6})-$/.exec(mails[1].headers.subject)[1]).as(
        'code',
      );
    });
    cy.get('@code').then((code) =>
      cy.get('#users-mfa-code').clear().type(code),
    );
    ...
    cy.get(
      '.com-users-methods-list-method-name-email a.com-users-methods-list-method-record-delete',
    ).click();
  ...
  it('can login with Multi-factor Authentication (totp)', () => {
    ...
    cy.get(
      '.com-users-methods-list-method-name-totp a.com-users-methods-list-method-addnew',
    ).click();
    cy.get('#com-users-method-edit-title').clear().type('Test Code');
    ...
    cy.get('@secret').then((secret) =>
      cy.get('#com-users-method-code').clear().type(TOTP.generate(secret).otp),
    );
    cy.get('#com-users-method-edit').submit();
    cy.get(
      '.com-users-methods-list-method-name-totp .com-users-methods-list-method-record',
    ).contains('Test Code');
    cy.clickToolbarButton('Cancel');
    cy.doAdministratorLogout();
    cy.get('#mod-login-username').type(Cypress.env('username'));
    ...
    cy.get('@secret').then((secret) =>
      cy.get('#users-mfa-code').clear().type(TOTP.generate(secret).otp),
    );
    cy.get('#users-mfa-captive-form').submit();
    cy.visit('/administrator/index.php?option=com_users&view=users');
    cy.get('.header-profile:visible').click();
    ...
    cy.get('#com-users-methods-reset-message').contains('is enabled');
    cy.get(
      '.com-users-methods-list-method-name-totp a.com-users-methods-list-method-record-delete',
    ).click();
    cy.on('window:confirm', (text) =>
      expect(text).to.contains('Are you sure you want to delete?'),
    );
  ...
  it(
    'can login with Multi-factor Authentication (passkey)',
    { browser: '!firefox' },
    () => {
      ...
      cy.visit('/administrator/index.php?option=com_users&view=users');
      cy.get('.header-profile:visible').click();
      cy.get('.header-profile a.dropdown-item')
      ...
      cy.get(
        '.com-users-methods-list-method-name-webauthn a.com-users-methods-list-method-addnew',
      ).click();
      cy.get('#com-users-method-edit-title').clear().type('Test Passkey');
      cy.get('#toolbar-user-mfa-edit-save').click();
      cy.get(
        '.com-users-methods-list-method-name-webauthn .com-users-methods-list-method-record',
      ).contains('Test Passkey');
      cy.clickToolbarButton('Cancel');
      cy.doAdministratorLogout();
      cy.get('#mod-login-username').type(Cypress.env('username'));
      cy.get('#mod-login-password').type(Cypress.env('password'));
      ...
      cy.get('#com-users-methods-reset-message').contains('is enabled');
      cy.get(
        '.com-users-methods-list-method-name-webauthn a.com-users-methods-list-method-record-delete',
      ).click();
      cy.on('window:confirm', (text) =>
        expect(text).to.contains('Are you sure you want to delete?'),
      );
  ...
  it('can login with Multi-factor Authentication (backup codes)', () => {
    ...
    cy.get(
      '.com-users-methods-list-method-name-backupcodes .com-users-methods-list-method-record-info a',
    )
    ...
    cy.get('table > tbody > tr > td')
      .first()
      .invoke('text')
      .then((code) => cy.wrap(/\d{8}/.exec(code)[0]).as('code'));
