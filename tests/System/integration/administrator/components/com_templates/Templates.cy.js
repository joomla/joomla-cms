describe('Test in backend that the templates list', () => {
  ...
  it('can list site templates', () => {
    ...
    cy.get('#templateList tbody .template-name a')
      .contains('Cassiopeia')
      .click();
    cy.get('h1.page-title').should(
      'contain.text',
      'Templates: Customise (Cassiopeia)',
    );
  ...
  it('can list administrator templates', () => {
    cy.visit(
      '/administrator/index.php?option=com_templates&view=templates&client_id=1',
    );
    cy.get('h1.page-title').should(
      'contain.text',
      'Templates: Templates (Administrator)',
    );
  ...
  it('can open options', () => {
    ...
    cy.intercept(
      '**/administrator/index.php?option=com_config&view=component&component=com_templates*',
    ).as('options');
    cy.intercept(
      '**/administrator/index.php?option=com_templates&view=templates&client_id=1*',
    ).as('listview');
