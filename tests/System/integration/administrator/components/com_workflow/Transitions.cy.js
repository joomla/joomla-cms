describe('Test in backend that the transitions list', () => {
  ...
  it('can enable the transition', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition', published: 0 }).then(() => {
      cy.reload();
  ...
  it('can disable the transition', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition', published: 1 }).then(() => {
      cy.reload();
  ...
  it('can delete the transition', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition', published: -2 }).then(() => {
      cy.reload();
  ...
  it('can checkin the transition', () => {
    cy.db_getUserId(Cypress.env('username')).then((uid) => {
      cy.db_createWorkflowTransition({ title: 'Test transition', checked_out: uid, checked_out_time: '2025-01-01 00:00:00' }).then(() => {
        cy.reload();
        cy.searchForItem('Test transition');
        cy.checkAllResults();
        cy.clickToolbarButton('Action');
  ...
  it('can enable the transition (grid button)', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition', published: 0, description: 'transition1' }).then(() => {
      cy.reload();
      cy.setFilter('published', 'Unpublished');
  ...
  it('can disable the transition (grid button)', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition', published: 1, description: 'transition2' }).then(() => {
      cy.reload();
      cy.setFilter('published', 'Published');
  ...
  it('can filter state', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition 1', published: 1 })
      .then(() => cy.db_createWorkflowTransition({ title: 'Test transition 2', published: 0 }))
      .then(() => cy.db_createWorkflowTransition({ title: 'Test transition 3', published: -2 }))
      .then(() => {
        cy.reload();
  ...
  it('can filter current stage', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition 1' })
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 2' }).then((stage) => cy.db_createWorkflowTransition({ title: 'Test transition 2', from_stage_id: stage.id })))
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 3' }).then((stage) => cy.db_createWorkflowTransition({ title: 'Test transition 3', from_stage_id: stage.id })))
      .then(() => {
        cy.reload();
  ...
  it('can filter target stage', () => {
    cy.db_createWorkflowTransition({ title: 'Test transition 1' })
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 2' }).then((stage) => cy.db_createWorkflowTransition({ title: 'Test transition 2', to_stage_id: stage.id })))
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 3' }).then((stage) => cy.db_createWorkflowTransition({ title: 'Test transition 3', to_stage_id: stage.id })))
      .then(() => {
        cy.reload();
