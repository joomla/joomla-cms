describe('Test in backend that the stages list', () => {
  ...
  it('can enable the stage', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', published: 0 }).then(() => {
      cy.reload();
  ...
  it('can disable the stage', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', published: 1 }).then(() => {
      cy.reload();
  ...
  it('can delete the stage', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', published: -2 }).then(() => {
      cy.reload();
  ...
  it('can checkin the stage', () => {
    cy.db_getUserId(Cypress.env('username')).then((uid) => {
      cy.db_createWorkflowStage({ title: 'Test stage', checked_out: uid, checked_out_time: '2025-01-01 00:00:00' }).then(() => {
        cy.reload();
        cy.searchForItem('Test stage');
        cy.checkAllResults();
        cy.clickToolbarButton('Action');
  ...
  it('can enable the stage (grid button)', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', published: 0, description: 'stage1' }).then(() => {
      cy.reload();
      cy.setFilter('published', 'Unpublished');
  ...
  it('can disable the stage (grid button)', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', published: 1, description: 'stage2' }).then(() => {
      cy.reload();
      cy.setFilter('published', 'Published');
  ...
  it('can set stage as default (grid button)', () => {
    cy.db_createWorkflowStage({ title: 'Test stage', default: 0 }).then(() => {
      cy.reload();
      cy.searchForItem('Test stage');
  ...
  it('can filter state', () => {
    cy.db_createWorkflowStage({ title: 'Test stage 1', published: 1 })
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 2', published: 0 }))
      .then(() => cy.db_createWorkflowStage({ title: 'Test stage 3', published: -2 }))
      .then(() => {
        cy.reload();
