describe('Test in backend that the privacy consent component', () => {
  ...
  it('can filter by valid consent', () => {
    ...
    cy.db_createUser().then((id) => {
      cy.db_createPrivacyConsent({
        state: 1,
        body: 'valid consent user',
        user_id: `${id}`,
      });
      cy.db_createPrivacyConsent({
        state: -1,
        body: 'invalidated consent user',
        user_id: `${id}`,
      });
      cy.db_createPrivacyConsent({
        state: 0,
        body: 'obsolete consent user',
        user_id: `${id}`,
      });
    });
  ...
  it('can list by consented in descending order', () => {
    ...
    cy.db_createUser().then((id) => {
      ...
      cy.db_createPrivacyConsent({
        state: -1,
        created: '2022-12-31 18:30:00',
        user_id: `${id}`,
      });
      cy.db_createPrivacyConsent({
        state: 0,
        created: '2024-02-15 12:45:00 ',
        user_id: `${id}`,
      });
    });
