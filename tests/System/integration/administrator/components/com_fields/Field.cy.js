describe('Test in backend that the field form', () => {
  ...
  it('can create a field', () => {
    cy.visit(
      '/administrator/index.php?option=com_fields&task=field.add&context=com_content.article',
    );
  ...
  it('can edit a field', () => {
    cy.db_createField({ title: 'Test field' }).then((id) => {
      cy.visit(
        `/administrator/index.php?option=com_fields&task=field.edit&id=${id}&context=com_content.article`,
      );
