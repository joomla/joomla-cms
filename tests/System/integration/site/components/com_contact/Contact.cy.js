describe('Test in frontend that the contact details view', () => {
  it('can display a form', () => {
    cy.db_getUserId()
      .then((id) => cy.db_createContact({ name: 'contact 1', user_id: id }))
      .then((contact) => {
  ...
  it('can display a custom field', () => {
    cy.db_createFieldGroup({
      title: 'automated test_field group',
      ...
      .then((id) =>
        cy.db_createField({
          ...
          fieldparams: JSON.stringify({
            options: { options0: { name: 'test value', value: '' } },
          }),
        }),
      )
