describe('Test in frontend that the users registration view', () => {
  ...
  afterEach(() => {
    ...
    cy.task(
      'queryDB',
      "DELETE FROM #__user_usergroup_map WHERE user_id = (SELECT id FROM #__users WHERE username = 'testuser')",
    );
  ...
  it('can display a registration form for a test user without a menu item', () => {
    ...
    cy.get('.com-users-registration__form').should(
      'contain.text',
      'User Registration',
    );
    cy.get('#jform_name').clear().type('test user');
    cy.get('#jform_username').clear().type('testuser');
    cy.get('#jform_password1')
      .clear()
      .type(`${Cypress.env('password')}-test`);
    cy.get('#jform_password2')
      .clear()
      .type(`${Cypress.env('password')}-test`);
    cy.get('#jform_email1').clear().type('testuser@example.com');
    cy.get('#member-registration').submit();
    cy.checkForSystemMessage(
      'Your account has been created and a verification link has been sent to the email address you entered.',
    );
    ...
    cy.task('getMails').then((mails) => {
      ...
      cy.wrap(mails[0].headers.subject).should(
        'have.string',
        `Account Details for test user at ${Cypress.env('sitename')}`,
      );
      ...
      cy.wrap(mails[0].body).should(
        'match',
        /http\S+\?task=registration\.activate&token=[a-z0-9]+/,
      );
      cy.wrap(
        /http\S+\?task=registration\.activate&token=[a-z0-9]+/.exec(
          mails[0].body,
        )[0],
      ).as('activatelink');
    ...
    cy.task('getMails').then((mails) => {
      ...
      cy.wrap(mails[1].headers.subject).should(
        'have.string',
        `Registration approval required for account of test user at ${Cypress.env('sitename')}`,
      );
    ...
    cy.task('getMails').then((mails) => {
      ...
      cy.wrap(mails[2].headers.subject).should(
        'have.string',
        `Account activated for test user at ${Cypress.env('sitename')}`,
      );
      cy.wrap(mails[2].headers.to).should('equal', 'testuser@example.com');
      cy.wrap(mails[2].body).should(
        'have.string',
  ...
  it('can display a registration form for a test user in a menu item', () => {
    cy.db_createMenuItem({
      title: 'Automated test registration',
