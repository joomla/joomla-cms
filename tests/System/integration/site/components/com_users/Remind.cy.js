describe('Test in frontend that the users remind view', () => {
  ...
  it('can send a reminder email for a test user without a menu item', () => {
    cy.db_createUser({ name: 'test user', email: 'test@example.com' }).then(
      () => {
        ...
        cy.task('getMails').then((mails) => {
          cy.checkForSystemMessage(
            'If the email address you entered is registered on this site you will shortly receive an email with a reminder.',
          );

          expect(mails.length).to.equal(1);
          cy.wrap(mails[0].body).should(
            'have.string',
            'A username reminder has been requested',
          );
          cy.wrap(mails[0].body).should(
            'have.string',
