    ...
    cy.db_createNewsFeed({ name: 'automated test feed 1' })
      .then((feed) =>
        cy.db_createNewsFeed({
          name: 'automated test feed 2',
          catid: feed.catid,
        }),
      )
      .then((feed) =>
        cy.db_createNewsFeed({
          name: 'automated test feed 3',
          catid: feed.catid,
        }),
      )
      .then((feed) =>
        cy.db_createNewsFeed({
          name: 'automated test feed 4',
          catid: feed.catid,
        }),
      )
      .then((feed) =>
        cy.db_createMenuItem({
          title: 'automated test feeds',
          link: `index.php?option=com_newsfeeds&view=category&id=${feed.catid}`,
        }),
      )
      .then(() => {
        cy.visit('/');
