describe('Test in frontend that the content site router', () => {
  afterEach(() =>
    cy.db_updateExtensionParameter('sef_ids', '1', 'com_content'),
  );
  ...
  it('can process article with a single article menu item', () => {
    ...
    cy.db_createArticle({
      ...
      cy.request({
        url: `/index.php?option=com_content&view=article&id=${article.id}&catid=${article.catid}`,
        followRedirect: false,
      }).then((response) => {
        expect(response.status).to.eq(301);
        expect(response.redirectedToUrl).to.match(new RegExp(`${url}$`));
      ...
      cy.request({
        url: `/index.php?option=com_content&view=article&id=${article.id}-${article.alias}&catid=${article.catid}`,
        followRedirect: false,
      }).then((response) => {
        expect(response.status).to.eq(301);
        expect(response.redirectedToUrl).to.match(new RegExp(`${url}$`));
  ...
  it('can process article with a category list menu item', () => {
    ...
    cy.db_createArticle({
      ...
      cy.get('@breadcrumb')
        .eq(2)
        .should('contain', 'Test Menu Article Category');
  ...
  it('can process article with a categories list menu item', () => {
    const url =
      '/index.php/test-menu-categories-router/uncategorised/test-content-router';
    cy.db_createArticle({
      title: 'Test Article',
      alias: 'test-content-router',
    }).then((article) => {
      ...
      cy.get('@breadcrumb')
        .eq(2)
        .should('contain', 'Test Menu Article Categories');
