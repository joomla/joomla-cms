describe('Test that console command user', () => {
  ...
  it('can add a user', () => {
    const para =
      '--username=test --name=test --password=123456789012 --email=test@530.test --usergroup=Manager -n';
  ...
  it('can reset password', () => {
    ...
    cy.exec(
      `php ${Cypress.env('cmsPath')}/cli/joomla.php user:reset-password ${para}`,
    )
      .its('stdout')
  ...
  it('can add a user to user group', () => {
    ...
    cy.exec(
      `php ${Cypress.env('cmsPath')}/cli/joomla.php user:addtogroup ${para}`,
    )
      .its('stdout')
  ...
  it('can remove a user from user group', () => {
    const para = '--username=test --group=Registered -n';
    cy.exec(
      `php ${Cypress.env('cmsPath')}/cli/joomla.php user:removefromgroup ${para}`,
    )
      .its('stdout')
