describe('Test privacy request API endpoint', () => {
  ...
  it('can list a single request', () => {
    cy.db_createPrivacyRequest()
      ...
      .then((response) => cy.wrap(response).its('body').its('data').its('attributes').its('status').should('equal', 0));
  });
  ...
  it('can create a request', () => {
    cy.api_post('/privacy/requests ', {
      ...
      request_type: 'export',
    }).then((response) => cy.wrap(response).its('body').its('data').its('attributes').its('email').should('include', 'test@example.com'));
  });
});
