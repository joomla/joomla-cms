describe('Test that menus site API endpoint', () => {
  ...
  it('can deliver a single site menu', () => {
    cy.db_createMenuType({ title: 'automated test site menu' })
      ...
      .then((response) => cy.wrap(response).its('body').its('data').its('attributes').its('title').should('include', 'automated test site menu'));
  });
  ...
  it('can update a site menu', () => {
    cy.db_createMenuType({ title: 'automated test site menu' })
      .then((id) =>
        cy.api_patch(`/menus/site/${id}`, {
          title: 'updated automated test site menu',
        }),
      )
      .then((response) =>
        cy.wrap(response).its('body').its('data').its('attributes').its('title').should('include', 'updated automated test site menu'),
      );
  });

  it('can delete a site menu', () => {
    cy.db_createMenuType({ title: 'automated test site menu' }).then((id) => cy.api_delete(`/menus/site/${id}`));
  });
});
