describe('Test that menus site API endpoint', () => {
  afterEach(() =>
    cy.task(
      'queryDB',
      "DELETE FROM #__menu_types WHERE title = 'automated test site menu' ",
    ),
  );
  ...
  it('can deliver a list of site menus', () => {
    cy.db_createMenuType({ title: 'automated test site menu' })
      ...
      .then((response) =>
        cy.api_responseContains(response, 'title', 'automated test site menu'),
      );
  });
  ...
  it('can delete a site menu', () => {
    cy.db_createMenuType({ title: 'automated test site menu' }).then((id) =>
      cy.api_delete(`/menus/site/${id}`),
    );
  });
});
