describe('Test that media files API endpoint', () => {
  ...
  beforeEach(() => {
    ...
    cy.task('writeRelativeFile', {
      path: 'files/test-dir/dummy.txt',
      content: '1',
    });
    // Ensure 'images/test-dir2' exists (relative to cmsPath) and has the correct permissions
    cy.task('writeRelativeFile', {
      path: 'images/test-dir2/dummy.txt',
      content: '1',
    });
    cy.task('copyRelativeFile', {
      source: `${mediaFixturesFolder}/test-image-1.jpg`,
      destination: 'files/test-image-1.jpg',
    });
    cy.task('copyRelativeFile', {
      source: `${mediaFixturesFolder}/test-image-2.jpg`,
      destination: 'files/test-dir/test-image-2.jpg',
    });
    cy.task('copyRelativeFile', {
      source: `${mediaFixturesFolder}/test-image-3.jpg`,
      destination: 'images/test-dir2/test-image-3.jpg',
    });
  });
  ...
  it('can deliver a list of files', () => {
    cy.api_get('/media/files').then((response) => {
  ...
  it('can create a file with adapter', () => {
    cy.readFile('tests/System/fixtures/com_media/test-image-2.jpg', 'binary')
      .then((data) =>
        cy.api_post('/media/files', {
          path: 'local-images:/test-dir2/test.jpg',
          content: Buffer.from(data, 'binary').toString('base64'),
        }),
      )
      .then((response) => {
  ...
  it('can update a file without adapter', () => {
    cy.task('writeRelativeFile', {
      path: 'files/test-dir/override.jpg',
      content: '1',
      mode: 0o666,
    })
  ...
  it('can update a folder without adapter', () => {
    cy.task('writeRelativeFile', {
      path: 'files/test-dir/override/test.jpg',
      content: '1',
      mode: 0o666,
    })
      .then(() =>
        cy.api_patch('/media/files/test-dir/override', {
          path: 'test-dir/override-new',
        }),
      )
      .then((response) => {
  ...
  it('can update a file with adapter', () => {
    cy.task('writeRelativeFile', {
      path: 'images/test-dir2/override.jpg',
      content: '1',
      mode: 0o666,
    })
  ...
  it('can update a folder with adapter', () => {
    cy.task('writeRelativeFile', {
      path: 'images/test-dir2/override/test.jpg',
      content: '1',
      mode: 0o666,
    })
      .then(() =>
        cy.api_patch('/media/files/local-images:/test-dir2/override', {
          path: 'local-images:/test-dir2/override-new',
        }),
      )
      .then((response) => {
        cy.wrap(response).its('body').its('data').its('attributes').its('name').should('include', 'override-new');
