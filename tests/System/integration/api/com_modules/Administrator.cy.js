describe('Test that modules administrator API endpoint', () => {
  ...
  it('can deliver a list of administrator modules', () => {
    cy.api_get('/modules/administrator').then((response) =>
      cy.wrap(response).its('body').its('data.0').its('attributes').its('module').should('include', 'mod_sampledata'),
    );
  });
  ...
  it('can deliver a single administrator module', () => {
    cy.db_createModule({
      ...
      .then((response) =>
        cy.wrap(response).its('body').its('data').its('attributes').its('title').should('include', 'automated test administrator module'),
      );
  });
