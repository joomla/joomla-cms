...
class JStringInflectorTest extends PHPUnit_Framework_TestCase
{
	...
	{
		// Case 1
		ReflectionHelper::invoke($this->inflector, '_addRule', '/foo/', 'singular');

		$rules = ReflectionHelper::getValue($this->inflector, '_rules');
		...
		// Case 2
		ReflectionHelper::invoke($this->inflector, '_addRule', '/bar/', 'plural');
		...
		// Case 3
		ReflectionHelper::invoke($this->inflector, '_addRule', array('/goo/', '/car/'), 'singular');
	...
	{
		ReflectionHelper::invoke($this->inflector, '_addRule', new stdClass, 'singular');
	}
	...
	{
		// Reset the cache.
		ReflectionHelper::setValue($this->inflector, '_cache', array('foo' => 'bar'));

		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_getCachedPlural', 'bar'),
			$this->isFalse(),
			'Checks for an uncached plural.'
		);

		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_getCachedPlural', 'foo'),
			$this->equalTo('bar'),
	...
	public function test_getCachedSingular()
	{
		...
		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_getCachedSingular', 'foo'),
			$this->isFalse(),
			'Checks for an uncached singular.'
		);

		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_getCachedSingular', 'bar'),
			$this->equalTo('foo'),
	...
	{
		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_matchRegexRule', 'xyz', 'plural'),
			$this->equalTo('xyzs'),
			'Checks pluralising against the basic regex.'
		);

		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_matchRegexRule', 'xyzs', 'singular'),
			$this->equalTo('xyz'),
			'Checks singularising against the basic regex.'
		);

		$this->assertThat(
			ReflectionHelper::invoke($this->inflector, '_matchRegexRule', 'xyz', 'singular'),
			$this->isFalse(),
	...
	{
		ReflectionHelper::invoke($this->inflector, '_setCache', 'foo', 'bar');

		$cache = ReflectionHelper::getValue($this->inflector, '_cache');
		...
		ReflectionHelper::invoke($this->inflector, '_setCache', 'foo', 'car');
	...
	public function testGetInstance()
	{
		...
		ReflectionHelper::setValue($this->inflector, '_instance', new stdClass);
