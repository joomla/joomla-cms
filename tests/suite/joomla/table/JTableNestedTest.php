...
class JTableNestedTest extends JoomlaDatabaseTestCase
{
	...
	{
		$this->class->debug(99);
		$this->assertEquals(99, ReflectionHelper::getValue($this->class, '_debug'));
	}
	...
	{
// 		ReflectionHelper::invoke($this->class, '_logtable');
	...
	public function testRebuild()
	{
		...
		ReflectionHelper::setValue($this->class, '_cache', array());
		$this->class->rebuild();
...
// 		ReflectionHelper::invoke($this->class, '_logtable');
	...
	{
		$node = ReflectionHelper::invoke($this->class, '_getNode', 1);
		...
		$node = ReflectionHelper::invoke($this->class, '_getNode', 103, 'parent');
		...
		$node = ReflectionHelper::invoke($this->class, '_getNode', 3, 'left');
		...
		$node = ReflectionHelper::invoke($this->class, '_getNode', 2, 'right');
		...
		$node = ReflectionHelper::invoke($this->class, '_getNode', 1, 'foo');
		$this->assertEquals(1, $node->id, 'Check id (by foo).');

		$node = ReflectionHelper::invoke($this->class, '_getNode', 99);
	...
	public function test_getTreeRepositionData()
	{
		...
		$before = ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 10, 'before');
		$this->assertEquals(
		...
		$after = ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 10, 'after');
		$this->assertEquals(
		...
		$firstChild = ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 10, 'first-child');
		$this->assertEquals(
		...
		$lastChild = ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 10, 'last-child');
		$this->assertEquals(
		...
		$default = ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 10);
		$this->assertEquals($default, $before, 'Checks the default handling is before.');

		$this->assertFalse(
			ReflectionHelper::invoke($this->class, '_getTreeRepositionData', 'foo', 10),
			'Checks an invalid data type.'
		);

		$this->assertFalse(
			ReflectionHelper::invoke($this->class, '_getTreeRepositionData', (object) array('lft' => 1), 10),
		...
		$this->assertFalse(
			ReflectionHelper::invoke($this->class, '_getTreeRepositionData', $object, 1),
	...
	public function test_runQuery()
	{
		...
		ReflectionHelper::invoke($this->class, '_runQuery', 'SELECT * FROM #__categories', 'foo');
		...
		try
		{
			...
			ReflectionHelper::invoke($class, '_runQuery', 'SELECT foo FROM #__categories', 'foo');
