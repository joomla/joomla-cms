...
class NewsFeedManager0002Test extends JoomlaWebdriverTestCase
{
  /**
	...
	public function setFilter_SetFilterValues_ShouldExecuteFilter()
	{
		...
		$this->assertEquals(1, $this->newsFeedManagerPage->getRowNumber($feedName), 'Feed should be in row 1');
		$this->newsFeedManagerPage->deleteItem($feedName);
	 ...
	 * @test
	 */
	public function setFilter_TestFilters_ShouldFilterFeed()
	{
		...
		$this->assertEquals('unpublished', $state, 'Initial state should be unpublished');
		...
		$this->assertEquals('unpublished', $state, 'Initial state should be unpublished');
		$this->newsFeedManagerPage->changeFeedState($feedName_2, 'Published');
		
		$test = $this->newsFeedManagerPage->setFilter('filter_published', 'Published');
		$this->assertFalse($this->newsFeedManagerPage->getRowNumber($feedName_1), 'NewsFeed should not show');
		$this->assertEquals(5, $this->newsFeedManagerPage->getRowNumber($feedName_2), 'NewsFeed should be in row 5');
		
		$test = $this->newsFeedManagerPage->setFilter('filter_published', 'Unpublished');
		$this->assertFalse($this->newsFeedManagerPage->getRowNumber($feedName_2), 'NewsFeed should not show');
		$this->assertEquals(1, $this->newsFeedManagerPage->getRowNumber($feedName_1), 'NewsFeed should be in row 1');
		
		$this->newsFeedManagerPage->setFilter('Select Status', 'Select Status');
		$this->newsFeedManagerPage->deleteItem($feedName_1);
		$this->newsFeedManagerPage->deleteItem($feedName_2);
	}
