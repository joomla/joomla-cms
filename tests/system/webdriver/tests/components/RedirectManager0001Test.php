  ...
  /**
	...
	*/
	...
	public function addRedirect_WithFieldDefaults_RedirectAdded()
	{
		...
		$this->redirectManagerPage->deleteItem($src);
		$this->assertFalse($this->redirectManagerPage->getRowNumber($src), 'Test link should not be present');
	}
	...
	*/
	public function editRedirect_ChangeFields_FieldsChanged()
	{
		...
		$this->assertFalse($this->redirectManagerPage->getRowNumber($src), 'Test link should not be present');
		$this->redirectManagerPage->addRedirect($src,$dest,$status,$comments);
		$this->redirectManagerPage->editRedirect($src, array('Destination URL' => 'new_sample_Destination', 'Comment' => 'New_Sample_Comment'));
		$values = $this->redirectManagerPage->getFieldValues('RedirectEditPage', $src, array('Destination URL', 'Comment'));
		$this->assertEquals(array('new_sample_Destination', 'New_Sample_Comment'), $values, 'Actual values should match expected');
		$this->redirectManagerPage->deleteItem($src);
	}
	...
	*/ 
	...
	{
		$this->redirectManagerPage->addRedirect('Test Redirect');
		$state = $this->redirectManagerPage->getState('Test Redirect');
		$this->assertEquals('published', $state, 'Initial state should be published');
		$this->redirectManagerPage->changeRedirectState('Test Redirect', 'unpublished');
		$state = $this->redirectManagerPage->getState('Test Redirect');
		$this->assertEquals('unpublished', $state, 'State should be unpublished');
		$this->redirectManagerPage->deleteItem('Test Redirect');
	}
