...
// 		'Australian Parks'		=> array('MenuItemsManagerPage', 'administrator/index.php?option=com_menus&view=items&menutype=parks', 'menus'),
// 		'Australian Parks Add New Menu Item'	=> array('MenuItemEditPage', 'administrator/index.php?option=com_menus&view=item&layout=edit&menutype=parks', 'menus'),
...
// 		$this->testPage = $this->doAdminLogin();
	...
	public function takeScreenShotsForModuleTypes()
	{
		...
		foreach ($moduleManagerPage->moduleTypes as $type)
		{
			...
			$moduleEditPage->setFieldValues(array(
				'Title' => $type['name']
			));
