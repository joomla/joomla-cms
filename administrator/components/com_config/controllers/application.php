...
class ConfigControllerApplication extends ConfigController
{
	...
	function showConfig()
	{
		...
		$lists['feed_summary']	= JHTML::_('select.radiolist', $summary, 'feed_summary', 'class="inputbox"', $row->feed_summary);
	...
	function save()
	{
		...
		if (!$ftp['enabled'] && !JPath::setPermissions($fname, '0644')) {
		...
		if (!$ftp['enabled'] && !JPath::setPermissions($fname, '0444')) {
