...
JLoader::register('SearchIndexer', JPATH_ADMINISTRATOR . '/components/com_search/helpers/indexer/indexer.php');

/**
 * Indexer controller class for Search.
 *
 * @since  2.5
 */
class SearchControllerIndexer extends JControllerLegacy
{
	/**
	...
	public function start()
	{
		...
		JText::script('COM_SEARCH_NO_ERROR_RETURNED');
	...
	public function optimize()
	{
		...
		{
			// Optimize the index
			SearchIndexer::getInstance()->optimize();
	...
	public static function sendResponse($data = null)
	{
		...
		$response = new SearchIndexerResponse($data);
...
/**
 * Search Indexer JSON Response Class
 *
 * @since  2.5
 */
class SearchIndexerResponse
{
	/**
	 * Class Constructor
	 *
	...
	public function __construct($state)
	{
		...
		if ($state instanceof Exception)
		{
			...
			$this->error = true;
			$this->header = JText::_('COM_SEARCH_INDEXER_HEADER_ERROR');
		...
		else
		{
			...
			{
				$this->header = JText::_('COM_SEARCH_INDEXER_HEADER_COMPLETE');
				$this->message = JText::_('COM_SEARCH_INDEXER_MESSAGE_COMPLETE');
			}
			elseif ($this->totalItems <= 0)
			{
				$this->header = JText::_('COM_SEARCH_INDEXER_HEADER_OPTIMIZE');
				$this->message = JText::_('COM_SEARCH_INDEXER_MESSAGE_OPTIMIZE');
			}
			else
			{
				$this->header = JText::_('COM_SEARCH_INDEXER_HEADER_RUNNING');
				$this->message = JText::_('COM_SEARCH_INDEXER_MESSAGE_RUNNING');
			}
		}
	}
}
...
JError::setErrorHandling(E_ALL, 'callback', array('SearchControllerIndexer', 'sendResponse'));
