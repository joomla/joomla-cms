...
JLoader::register('SearchHelperLanguage', JPATH_ADMINISTRATOR . '/components/com_finder/helpers/lang.php');
		...
		if ($contentMap)
		{
			...
			foreach ($contentMap as $branch)
			{
				...
				else
				{
					...
					{
						$text = SearchHelperLanguage::branchLanguageTitle($branch->text);
					}
					else
					{
						$key = SearchHelperLanguage::branchSingular($branch->text);
