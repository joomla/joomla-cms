...
JLoader::register('SearchHelperRoute', JPATH_SITE . '/components/com_finder/helpers/route.php');
JLoader::register('SearchHelperLanguage', JPATH_ADMINISTRATOR . '/components/com_search/helpers/lang.php');

/**
 * Query class for the Search indexer package.
 *
 * @since  2.5
 */
class SearchIndexerQuery
{
	/**
	...
	public function __construct($options)
	{
		...
		$this->language = !empty($options['language']) ? $options['language'] : SearchIndexerHelper::getDefaultLanguage();
		$this->language = SearchIndexerHelper::getPrimaryLanguage($this->language);
	...
	public function toUri($base = null)
	{
		...
		if (!$uri->getVar('Itemid'))
		{
			...
			$item = SearchHelperRoute::getItemid($query);
	...
	protected function processString($input, $lang, $mode)
	{
		...
		foreach (SearchIndexerTaxonomy::getBranchTitles() as $branch)
		{
			// Add the pattern.
			$patterns[$branch] = JString::strtolower(JText::_(SearchHelperLanguage::branchSingular($branch)));
		}
			...
			if (preg_match('#' . $pattern . '\s*:\s*' . $suffix . '#mi', $input, $matches))
			{
				...
				switch ($modifier)
				{
					...
					default:
						...
						$return = SearchIndexerTaxonomy::getNodeByTitle($modifier, $value);
		...
		for ($i = 0, $c = count($terms); $i < $c; $i++)
		{
			...
			if (isset($terms[$i + 1]) && in_array($terms[$i + 1], $operators))
			{
				...
				if ($op === 'AND' && isset($terms[$i + 2]))
				{
					...
					$token = SearchIndexerHelper::tokenize($terms[$i], $lang, true);
					...
					$other = SearchIndexerHelper::tokenize($terms[$i + 2], $lang, true);
			...
			elseif (isset($terms[$i + 1]) && array_search($terms[$i], $operators) === 'OR')
			{
				...
				$other = SearchIndexerHelper::tokenize($terms[$i + 1], $lang, true);
				$other = $this->getTokenData($other);
		...
		{
			// Tokenize the phrase.
			$token = SearchIndexerHelper::tokenize($phrases[$i], $lang, true);
			$token = $this->getTokenData($token);
		...
		if (!empty($terms))
		{
			// Tokenize the terms.
			$terms = implode(' ', $terms);
			$tokens = SearchIndexerHelper::tokenize($terms, $lang, false);
	 ...
	 *
	 * @param   SearchIndexerToken  $token  A SearchIndexerToken object.
	 *
	 * @return  SearchIndexerToken  A SearchIndexerToken object.
	 *
	 * @since   2.5
	 * @throws  Exception on database error.
	 */
