...
/**
 * Helper class for the Search indexer package.
 *
 * @since  2.5
 */
class SearchIndexerHelper
{
	 ...
	 * wishes to use it but it must be an instance of SearchIndexerStemmer.
	 *
	 * @var		SearchIndexerStemmer
	 * @since	2.5
	 */
	...
	public static function parse($input, $format = 'html')
	{
		...
		return SearchIndexerParser::getInstance($format)->parse($input);
	}
	 ...
	 *
	 * @return  array  An array of SearchIndexerToken objects.
	 *
	 * @since   2.5
	 */
	public static function tokenize($input, $lang, $phrase = false)
	{
		...
		if ($phrase === true && count($terms) > 1)
		{
			...
			$tokens[] = new SearchIndexerToken($terms, $lang);
		}
		else
		{
			...
			for ($i = 0, $n = count($terms); $i < $n; $i++)
			{
				$tokens[] = new SearchIndexerToken($terms[$i], $lang);
			}
			...
			for ($i = 0, $n = count($tokens); $i < $n; $i++)
			{
				...
				if ($i2 < $n && isset($tokens[$i2]))
				{
					...
					$token = new SearchIndexerToken(array($tokens[$i]->term, $tokens[$i2]->term), $lang, $lang === 'zh' ? '' : ' ');
					$token->derived = true;
				...
				if ($i3 < $n && isset($tokens[$i3]))
				{
					...
					$token = new SearchIndexerToken(array($tokens[$i]->term, $tokens[$i2]->term, $tokens[$i3]->term), $lang, $lang === 'zh' ? '' : ' ');
					$token->derived = true;
	 ...
	 * {@link SearchIndexerHelper::$stemmer} object if it is set. If no stemmer is set,
	...
	public static function stem($token, $lang)
	{
		...
		if (static::$stemmer instanceof SearchIndexerStemmer)
		{
	 ...
	 * we add Comments, Tags, Labels, etc. that should be available to Search.
	 *
	 ...
	 * @throws  Exception on database error.
	 */
	public static function getContentExtras(SearchIndexerResult &$item)
	{
