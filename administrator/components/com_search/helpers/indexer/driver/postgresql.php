...
/**
 * Indexer class supporting PostgreSQL for the Search indexer package.
 *
 * @since  3.0
 */
class SearchIndexerDriverPostgresql extends SearchIndexer
{
	/**
	 * Method to index a content item.
	 *
	 * @param   SearchIndexerResult  $item    The content item to index.
	...
	public function index($item, $format = 'html')
	{
		...
		if (!$isNew)
		{
			...
			SearchIndexerTaxonomy::removeMaps($linkId);
		}
		...
		foreach ($item->getTaxonomy() as $branch => $nodes)
		{
			...
			foreach ($nodes as $node)
			{
				...
				$nodeId = SearchIndexerTaxonomy::addNode($branch, $node->title, $node->state, $node->access);

				// Add the link => node map.
				SearchIndexerTaxonomy::addMap($linkId, $nodeId);
	...
	public function remove($linkId)
	{
		...
		SearchIndexerTaxonomy::removeOrphanNodes();
	 ...
	 *
	 * @param   mixed  $tokens   An array or single SearchIndexerToken object.
