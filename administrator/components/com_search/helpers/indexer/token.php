...
class SearchIndexerToken
{
	...
	public function __construct($term, $lang, $spacer = ' ')
	{
		...
		if (is_array($term))
		{
			...
			$this->stem = implode($spacer, array_map(array('SearchIndexerHelper', 'stem'), $term, array($lang)));
			$this->numeric = false;
			$this->common = false;
			$this->phrase = true;
		...
		else
		{
			// Populate the token instance.
			$this->term = $term;
			$this->stem = SearchIndexerHelper::stem($this->term, $lang);
