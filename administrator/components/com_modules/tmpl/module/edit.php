...
<form action="<?php echo Route::_('index.php?option=com_modules&layout=' . $layout . $tmpl . '&client_id=' . $this->form->getValue('client_id') . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="module-form" aria-label="<?php echo $this->text('COM_MODULES_FORM_TITLE_' . ((int) $this->item->id === 0 ? 'NEW' : 'EDIT'), true); ?>" class="form-validate">
    ...
    <div class="main-card">
        ...
        <?php echo HTMLHelper::_('uitab.addTab', 'myTab', 'general', $this->text('COM_MODULES_MODULE')); ?>
            ...
            <div class="col-lg-9">
                <?php if ($this->item->xml) : ?>
                    <?php if ($this->item->xml->description) : ?>
                        <h2>
                            <?php
                            if ($this->item->xml) {
                                echo ($text = (string) $this->item->xml->name) ? $this->text($text) : $this->item->module;
                            } else {
                                echo $this->text('COM_MODULES_ERR_XML');
                            }
                            ?>
                        </h2>
                ...
                <?php else : ?>
                    <div class="alert alert-danger">
                        ...
                        <?php echo $this->text('COM_MODULES_ERR_XML'); ?>
                    </div>
                <?php endif; ?>
                <?php
                if ($hasContent) {
        ...
        <?php if ($this->item->client_id == 0) : ?>
            <?php echo HTMLHelper::_('uitab.addTab', 'myTab', 'assignment', $this->text('COM_MODULES_MENU_ASSIGNMENT')); ?>
            <fieldset id="fieldset-assignment" class="options-form">
                <legend><?php echo $this->text('COM_MODULES_MENU_ASSIGNMENT'); ?></legend>
                <div>
        ...
        <?php if ($this->canDo->get('core.admin')) : ?>
            <?php echo HTMLHelper::_('uitab.addTab', 'myTab', 'permissions', $this->text('COM_MODULES_FIELDSET_RULES')); ?>
            <fieldset id="fieldset-permissions" class="options-form">
                <legend><?php echo $this->text('COM_MODULES_FIELDSET_RULES'); ?></legend>
                <div>
