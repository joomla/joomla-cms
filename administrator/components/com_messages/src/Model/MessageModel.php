...
class MessageModel extends AdminModel
{
			...
			if ($this->item = parent::getItem($pk))
			{
				...
				if (empty($this->item->message_id))
				{
					...
					if ($replyId = (int) $this->getState('reply.id'))
					{
						...
						if (!$message || $message->user_id_to != Factory::getApplication()->getIdentity()->id)
						{
							$this->setError(Text::_('JERROR_ALERTNOAUTHOR'));
				...
				elseif ($this->item->user_id_to != Factory::getApplication()->getIdentity()->id)
				{
					$this->setError(Text::_('JERROR_ALERTNOAUTHOR'));
	...
	public function save($data)
	{
		...
		{
			$table->user_id_from = Factory::getApplication()->getIdentity()->get('id');
		}
