...
<?php if (count($this->warnings)) : ?>
    <h3><?php echo $this->text('COM_INSTALLER_SUBMENU_WARNINGS'); ?></h3>
...
<form enctype="multipart/form-data" action="index.php" method="post" id="uploadForm">
    ...
    <div class="mb-3">
        <label for="install_package" class="form-label">
            <?php echo $this->text('COM_JOOMLAUPDATE_VIEW_UPLOAD_PACKAGE_FILE'); ?>
        </label>
        ...
        <div class="alert alert-warning hidden" id="max_upload_size_warn">
            <?php echo $this->text('COM_INSTALLER_MSG_WARNINGS_UPLOADFILETOOBIG'); ?>
        </div>
    </div>
    ...
    <div class="form-check mb-3 <?php echo $this->noBackupCheck ? 'd-none' : '' ?>">
        ...
        <label class="form-check-label" for="joomlaupdate-confirm-backup">
            <?php echo $this->text('COM_JOOMLAUPDATE_UPDATE_CONFIRM_BACKUP'); ?>
        </label>
    </div>

    <button id="uploadButton" class="btn btn-primary" disabled type="button"><?php echo $this->text('COM_INSTALLER_UPLOAD_AND_INSTALL'); ?></button>
