...
if ($saveOrder) {
    $saveOrderingUrl = 'index.php?option=com_jed&task=ticketmessages.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
...
<form action="<?php echo Route::_('index.php?option=com_jed&view=ticketmessages'); ?>" method="post"
                ...
                <table class="table table-striped" id="ticketmessageList">
                    ...
                    <?php foreach ($this->items as $i => $item) :
                        ...
                        <tr class="row<?php echo $i % 2; ?>">
                            ...
                            <?php if (isset($this->items[0]->state)) : ?>
                                <td class="center">
                                    <?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'ticketmessages.', $canChange, 'cb'); ?>
                                </td>
                            <?php endif; ?>
                            ...
                            <td>
                                <?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
                                    <?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'ticketmessages.', $canCheckin); ?>
                                <?php endif; ?>
                                <?php if ($canEdit) : ?>
                                    <a href="<?php echo Route::_('index.php?option=com_jed&task=ticketmessage.edit&id=' . (int) $item->id); ?>">
