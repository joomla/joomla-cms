...
class CjForumModelUsers extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// join over #__cjforum_users table
		$query
			->select(
					'cju.handle, cju.about, cju.avatar, cju.birthday, cju.topics, cju.replies, cju.thankyou, cju.points,'.
					'cju.fans, cju.hits, cju.last_post_time, cju.checked_out, cju.checked_out_time, cju.last_post_time')
			->join('LEFT', '#__cjforum_users AS cju on cju.id = a.id');
		
		// join over #__cjforum_ranks table
		$query
			->select('cjr.title as rank_title, cjr.alias as rank_alias')
			->join('LEFT', '#__cjforum_ranks AS cjr on cju.rank = cjr.id');
		...
		$rank = (int) $this->getState('filter.rank');
		
		if($rank)
		{
			$query->where('cju.rank = '.$rank);
		}
...
// echo $query->dump();
	...
	public function syncUsers($startId, $endId)
	{
		...
		{
			$query =
				'insert ignore into #__cjforum_users (id, handle, points) '.
				'select u.id, replace(u.username, \'-\', \'_\'), sum(p.points) as points '.
				'from #__users AS u '.
				'left join #__cjforum_points AS p on p.user_id = u.id '.
...
// 			throw new Exception($e); 
		...
		{
			$query = $db->getQuery(true)
				->update('#__cjforum_users AS u')
				->set('points = (select sum(p.points) from #__cjforum_points AS p where p.user_id = u.id group by p.user_id)')
				->set('topics = (select count(*) from #__cjforum_topics AS t where t.created_by = u.id group by t.created_by)')
				->set('replies = (select count(*) from #__cjforum_replies AS r where r.created_by = u.id group by r.created_by)')
				->where('u.id >= '.$startId.' and u.id <= '.$endId);
