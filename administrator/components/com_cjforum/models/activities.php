...
class CjForumModelActivities extends JModelList
{
	...
	protected function populateState ($ordering = null, $direction = null)
	{
		...
		$activityId = $this->getUserStateFromRequest($this->context . '.filter.category_id', 'filter_activity_id');
	...
	protected function getListQuery ()
	{
		...
		$query->select(
				$this->getState('list.select', 
						'a.id, a.title, a.description, a.created_by, a.created, a.activity_type, a.item_id, a.access,'.
						'a.parent_id, a.published, a.featured, a.likes, a.dislikes, a.checked_out, a.checked_out_time, a.language'));
		
		$query->from('#__cjforum_activity AS a');
		
		// Join over activity types table
		$query
			->select('at.title AS activity_type_title, at.description AS activity_type_description, at.app_name, at.activity_name, at.published as activity_type_state')
			->join('left', '#__cjforum_activity_types AS at on at.id = a.activity_type');
...
// 		echo $query->dump();
	...
	public function getAuthors ()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__cjforum_activity AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
