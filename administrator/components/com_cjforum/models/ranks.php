...
class CjForumModelRanks extends JModelList
{
	...
	protected function getListQuery ()
	{
		...
		$query->select(
				$this->getState('list.select', 
						'a.id, a.title, a.alias, a.checked_out, a.checked_out_time, a.catid, a.rank_type, a.min_posts, a.rank_image, a.rank_class,' .
						'a.state, a.access, a.created, a.created_by, a.created_by_alias, a.ordering, a.featured, a.language, a.hits, a.publish_up, a.publish_down'));
		$query->from('#__cjforum_ranks AS a');
			...
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join('LEFT', 
					$db->quoteName('#__contentitem_tag_map', 'tagmap') . ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' .
							 $db->quoteName('a.id') . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_cjforum.rank'));
		}
	...
	public function getAuthors ()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__cjforum_ranks AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	public function getItems ()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove ranks the user shouldn't see
	...
	public function syncRanks($startId, $endId)
	{
		$db = JFactory::getDbo();
		...
		{
			$query = $db->getQuery(true)
				->select('id, rank, topics+replies AS posts')
				->from('#__cjforum_users')
				->where('id >= '.((int)$startId))
				->where('id <= '.((int)$endId))
				->where('rank in (select r.id from #__cjforum_ranks r where r.rank_type = 0)');
			...
			$query = $db->getQuery(true)->update('#__cjforum_users');
			$updates = array();
			$userIds = array();
			...
			foreach ($users as $user)
			{
				...
				{
					if($user->posts >= $rank->min_posts)
					{
						$updates[] = 'when '.$user->id.' then '.$rank->id;
						$userIds[] = $user->id;
					}
				}
			}
			...
			if(!empty($updates))
			{
				$query->set('rank = case id '.implode(' ', $updates).' end');
