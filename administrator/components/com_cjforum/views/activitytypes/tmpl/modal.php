...
$function  = 'jSelectActivitytype_' . $field;
$listOrder = $this->escape($this->state->get('list.ordering'));
$listDirn  = $this->escape($this->state->get('list.direction'));
?>
<form action="<?php echo JRoute::_('index.php?option=com_cjforum&view=activitytypes&layout=modal&tmpl=component&function='.$function.'&'.JSession::getFormToken().'=1');?>" method="post" name="adminForm" id="adminForm">
	<fieldset class="filter">
		<div id="filter-bar" class="btn-toolbar">
			...
			<div class="btn-group pull-left">
				...
				<button type="button" class="btn" onclick="if (window.parent) window.parent.<?php echo $this->escape($function); ?>('', '<?php echo JText::_('COM_CJFORUM_SELECT_A_TYPE'); ?>');">
					<?php echo JText::_('COM_CJFORUM_OPTION_NO_TYPE'); ?>
				</button>
			</div>
		</div>
	</fieldset>
	...
	<table class="table table-striped table-condensed">
		<thead>
			<tr>
				...
				<th class="nowrap" width="25%">
					<?php echo JHtml::_('grid.sort', 'COM_CJFORUM_ACTIVITY_TYPE_NAME', 'a.activity_name', $listDirn, $listOrder); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<?php
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td><?php echo $this->escape($item->activity_name); ?></td>
			</tr>
		<?php endforeach; ?>
		</tbody>
	</table>
