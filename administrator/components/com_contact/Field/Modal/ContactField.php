 ...
 * @since  1.6
 */
class ContactField extends ModalField
{
	/**
	 * The form field type.
	 *
	...
	protected function getUrls()
	{
		...
		return [
			'select' => $linkContacts . '&amp;function=jModalSelect',
			// The edit url needs the id of the selected item. It will be inserted dynamically in place of ${0}
			'edit'   => $linkContact . '&amp;task=contact.edit&amp;id=${0}',
			'new'    => $linkContact . '&amp;task=contact.add',
		];
	}
	...
	{
		$selectTitle = Text::_('COM_CONTACT_CHANGE_CONTACT') . (isset($this->element['language']) ? ' &#8212; ' . $this->element['label'] : '');

		return array_merge(parent::getText(), [
			'title-select' => $selectTitle,
			'title-new'    => Text::_('COM_CONTACT_NEW_CONTACT'),
			'title-edit'   => Text::_('COM_CONTACT_EDIT_CONTACT'),
			'placeholder'  => Text::_('COM_CONTACT_CHANGE_CONTACT'),
		]);
	}
	...
	protected function getSelectedTitle()
	{
		...
		return empty($title) ? Text::_('COM_CONTACT_SELECT_A_CONTACT') : htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
	}
