...
abstract class FinderIndexerAdapter extends JPlugin
{
	...
	protected function checkItemAccess($row)
	{
		$query = $this->db->getQuery(true)
			...
			->where($this->db->quoteName($this->identifier_field) . ' = ' . (int) $row->id);
		$this->db->setQuery($query);
	...
	protected function getItem($id)
	{
		...
		$query = $this->getListQuery();
		$query->where('a.' . $this->identifier_field . ' = ' . (int) $id);
	...
	{
		$query = clone $this->getStateQuery();
		$query->where('a.' . $this->identifier_field . ' = ' . (int) $row->id);
	...
	protected function itemStateChange($pks, $value)
	{
		...
		{
			$query = clone $this->getStateQuery();
			$query->where('a.' . $this->identifier_field . ' = ' . (int) $pk);
