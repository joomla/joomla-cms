	...
	public function index($item, $format = 'html')
	{
		...
		if ($isNew)
		{
			...
			$query->values(
				...
				. $db->quote((double) ($item->list_price ? $item->list_price : 0)) . ', '
				. $db->quote((double) ($item->sale_price ? $item->sale_price : 0))
			);
			$db->setQuery($query);
			$db->execute();
		...
		else
		{
			...
			$query->set($db->qn('list_price') . ' = ' . $db->quote((double) ($item->list_price ? $item->list_price : 0)));
			$query->set($db->qn('sale_price') . ' = ' . $db->quote((double) ($item->sale_price ? $item->sale_price : 0)));
