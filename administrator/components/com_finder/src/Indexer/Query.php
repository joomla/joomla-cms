...
class Query
{
    ...
    public function toUri($base = '')
    {
        ...
        if (!$uri->getVar('Itemid')) {
            ...
            $query = ['view' => $uri->getVar('view'), 'f'    => $uri->getVar('f'), 'q'    => $uri->getVar('q')];
    ...
    protected function processString($input, $lang, $mode)
    {
         ...
         */
        $patterns = ['before' => Text::_('COM_FINDER_FILTER_WHEN_BEFORE'), 'after'  => Text::_('COM_FINDER_FILTER_WHEN_AFTER')];
        ...
        $operators = ['AND' => StringHelper::strtolower(Text::_('COM_FINDER_QUERY_OPERATOR_AND')), 'OR'  => StringHelper::strtolower(Text::_('COM_FINDER_QUERY_OPERATOR_OR')), 'NOT' => StringHelper::strtolower(Text::_('COM_FINDER_QUERY_OPERATOR_NOT'))];
    ...
    protected function getTokenData($token)
    {
        ...
        if ((bool) $matches) {
            ...
            for ($i = 0, $c = is_countable($matches) ? count($matches) : 0; $i < $c; $i++) {
