 ...
 * @since       1.6
 */
class Aa4jModelUsers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$hscore = $this->getUserStateFromRequest($this->context.'.filter.hscore', 'filter_hscore');
		$this->setState('filter.hscore', $hscore);
	...
	protected function getStoreId($id = '')
	{
		...
		$id	.= ':'.$this->getState('filter.hscore');
		$id	.= ':'.$this->getState('filter.state');
		$id	.= ':'.$this->getState('filter.group_id');
		$id .= ':'.$this->getState('filter.nation');
    ...
    $query->join('',$db->nameQuote('#__userextras').' AS b ON b.id = a.id');
		...
		// If the model is set to check the spam state, add to the query.
		$hscore = $this->getState('filter.hscore');
  //  var_dump('Hscore:'.$hscore);
		if (is_numeric($hscore))
		{
			if ($hscore == '0')
			{
				$query->where('b.hscore > 3 ');
			}
			elseif ($hscore == '1')
			{
				$query->where('b.hscore < 4 ');
			}
		}
  ...
  // var_dump($nation);
		...
		$query->order($db->escape($this->getState('list.ordering', 'a.registerDate')).' '.$db->escape($this->getState('list.direction', 'DESC')));
   echo ($query);
		return $query;
	}
	//sqlsrv change
