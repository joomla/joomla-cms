...
class ContenthistoryHelper
{
	 ...
	 * @param	integer   $typeId  ucm_type_id to find the type alias in the #__content_types table.
	 *
	 ...
	 * @param  array      $labelsArray    Associative array of id => label
	 *
	...
	{
		$object = ContenthistoryHelper::decodeFields($table->version_data);
		$typesTable = JTable::getInstance('Contenttype');
		$typesTable->load(array('type_id' => $table->ucm_type_id));
		$formValues = ContenthistoryHelper::getFormValues($object, $typesTable);
		$object = ContenthistoryHelper::mergeLabels($object, $formValues);
	 ...
	 *
	 * @param  stdClass            $object  The std object from the JSON string. Can be nested 1 level deep.
	 * @param  JTableContenttype   $table   Table object loaded with data.
	 *
