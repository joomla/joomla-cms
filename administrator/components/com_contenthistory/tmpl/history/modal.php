    ...
    <form action="<?php echo Route::_($formUrl); ?>" method="post" name="adminForm" id="adminForm">
        <table class="table table-sm">
            <caption class="visually-hidden">
                <?php echo $this->text('COM_CONTENTHISTORY_VERSION_CAPTION'); ?>
            </caption>
            <thead>
                <tr>
                    <td class="w-1 text-center">
                        <input class="form-check-input" type="checkbox" name="checkall-toggle" value="" title="<?php echo $this->text('JGLOBAL_CHECK_ALL'); ?>" onclick="Joomla.checkAll(this)">
                    </td>
                    <th scope="col" class="w-15">
                        <?php echo $this->text('JDATE'); ?>
                    </th>
                    <th scope="col" class="w-15 d-none d-md-table-cell">
                        <?php echo $this->text('COM_CONTENTHISTORY_VERSION_NOTE'); ?>
                    </th>
                    <th scope="col" class="w-10">
                        <?php echo $this->text('COM_CONTENTHISTORY_KEEP_VERSION'); ?>
                    </th>
                    <th scope="col" class="w-15 d-none d-md-table-cell">
                        <?php echo $this->text('JAUTHOR'); ?>
                    </th>
                    <th scope="col" class="w-10 text-end">
                        <?php echo $this->text('COM_CONTENTHISTORY_CHARACTER_COUNT'); ?>
                    </th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr class="row<?php echo $i % 2; ?>">
                        ...
                        <th scope="row">
                            <a href="#" class="js-link-open-window save-date" data-url="<?php echo Route::_('index.php?option=com_contenthistory&view=preview&layout=preview&tmpl=component&' . Session::getFormToken() . '=1&version_id=' . $item->version_id); ?>">
                                <?php echo HTMLHelper::_('date', $item->save_date, $this->text('DATE_FORMAT_LC6')); ?>
                            </a>
                            <?php if ($item->sha1_hash == $hash) : ?>
                                <span class="icon-star" aria-hidden="true"></span><span class="visually-hidden"><?php echo $this->text('JCURRENT'); ?></span>
                            <?php endif; ?>
                        </th>
                        ...
                        <td class="text-end">
                            <?php echo number_format((int) $item->character_count, 0, $this->text('DECIMALS_SEPARATOR'), $this->text('THOUSANDS_SEPARATOR')); ?>
                        </td>
                    </tr>
                    <?php $i++; ?>
                <?php endforeach; ?>
