			...
			?>
				<tr class="row<?php echo $i % 2; ?>">
					...
					<?php if (strpos($row->version, $minorVersion) !== 0 || strpos($row->version, $currentShortVersion) !== 0) : ?>
						<span class="label label-warning hasTooltip" title="<?php echo JText::_('JGLOBAL_LANGUAGE_VERSION_NOT_PLATFORM'); ?>"><?php echo JHtml::_('inlineBidirectional', $row->version, 'ltr'); ?></span>
					<?php else : ?>
						<span class="label label-success"><?php echo JHtml::_('inlineBidirectional', $row->version, 'ltr'); ?></span>
					<?php endif; ?>
					</td>
					<td class="hidden-phone">
