...
class JFormFieldModal_Newsfeed extends JFormField
{
	...
	protected function getInput()
	{
		...
		if ($allowEdit && !$scriptEdit)
		{
			...
			$script[] = '	function jEditNewsfeed(id) {';
			$script[] = '		SqueezeBox.open(null, {';
			$script[] = '			handler: "iframe",';
			$script[] = '			url: "index.php?option=com_newsfeeds&layout=modal&tmpl=component&task=newsfeed.edit&id=" + id,';
		...
		$html[] = '<a class="modal btn" title="'.JText::_('COM_NEWSFEEDS_CHANGE_FEED_BUTTON').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '.JText::_('JSELECT').'</a>';

		// Edit newsfeed button
		if ($allowEdit)
		{
			$html[] = '<button id="'.$this->id.'_edit" class="btn'.($value ? '' : ' hidden').'" title="'.JText::_('COM_NEWSFEEDS_EDIT_NEWSFEED').'" onclick="return jEditNewsfeed(jQuery(\'#'.$this->id.'_id\').val())"><span class="icon-edit"></span> ' . JText::_('JACTION_EDIT') . '</button>';
		}
