...
function messageForm( $option )
{
	...
	$gtree = array(
		JHTML::makeOption( 0, '- '. JText::_( 'All User Groups' ) .' -' )
	);

	// get list of groups
	$lists = array();
	$gtree = array_merge( $gtree, $acl->get_group_children_tree( null, 'USERS', false ) );
	$lists['gid'] = JHTML::selectList( $gtree, 'mm_group', 'size="10"', 'value', 'text', 0 );
...
function sendMail()
{
	...
	if (!$message_body || !$subject || $gou === null) {
		$mainframe->redirect( 'index.php?option=com_massmail&josmsg='. JText::_( 'Please fill in the form correctly' ) );
	}
