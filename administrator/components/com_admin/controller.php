...
class AdminController extends JControllerLegacy
{
	...
	public function download()
	{
		...
		$settingsToLoad = array('PhpSettings', 'Config', 'Info', 'Editor');
		...
		foreach($phpInfo as $section => $values) {
			foreach ( $values as $name => $setting ) {
				if(in_array($setting, $remove)) {
					$value = strlen($name) ? 'set' : 'not set';
				}
				$settings['PhpInfo'][$section][$name] = $value;
			}
		}
	...
	protected function loadSetting($section, $model)
	{
		...
		static $remove = array('open_basedir', 'session.save_path', 'mailfrom', 'fromname', 'smtphost', 'log_path', 'tmp_path', 'proxy_host', 'proxy_user', 'proxy_pass', 'memcache_server_host', 'memcached_server_host', 'session_memcache_server_host', 'session_memcached_server_host');
		$settings = array();
		$method = 'get'.$section;
