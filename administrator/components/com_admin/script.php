...
class JoomlaInstallerScript
{
	...
	public function postflight($action, $installer)
	{
		if ($action === 'update')
		{
			...
			if (!empty($this->fromVersion) && version_compare($this->fromVersion, '3.9.0', 'lt'))
			{
				/*
				 * Do a check if the menu item exists, skip if it does. Only needed when we are in pre stable state.
				 */
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__menu'))
					->where($db->quoteName('menutype') . ' = ' . $db->quote('main'))
					->where($db->quoteName('title') . ' = ' . $db->quote('com_privacy'))
					->where($db->quoteName('client_id') . ' = 1')
					->where($db->quoteName('component_id') . ' = 35');
				...
				/*
				 * Add a menu item for com_privacy, we need to do that here because with a plain sql statement we
				 * damage the nested set structure for the menu table
				 */
				$newMenuItem = JTable::getInstance('Menu');
				...
				$data              = array();
				$data['menutype']  = 'main';
				$data['title']     = 'com_privacy';
				$data['alias']     = 'Privacy';
				$data['path']      = 'Privacy';
				$data['link']      = 'index.php?option=com_privacy';
				$data['type']      = 'component';
				$data['published'] = 1;
				$data['parent_id'] = 1;

				// We have used a SQL Statement to add the extension so using 35 is safe (fingers crossed)
				$data['component_id'] = 35;
				$data['img']          = 'class:privacy';
				$data['language']     = '*';
				$data['client_id']    = 1;
