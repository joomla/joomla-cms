...
class JoomlaInstallerScript
{
	...
	protected function updateDatabase()
	{
		...
		// Convert plg_system_httpheader settings if installed
		$db = JFactory::getDbo();

		try
		{
			// Get the params for the httpheader plugin
			$id = $db->setQuery(
				$db->getQuery(true)
					->select($db->quoteName('extension_id'))
					->from($db->quoteName('#__extensions'))
					->where($db->quoteName('extension_id') . ' > 9999')
					->where($db->quoteName('type') . ' = ' . $db->quote('plugin'))
					->where($db->quoteName('folder') . ' = ' . $db->quote('system'))
					->where($db->quoteName('element') . ' = ' . $db->quote('httpheader'))
			)->loadResult();
		}
		catch (Exception $e)
		{
		...
		// Just do this if the plugin was installed
		if (!empty($id))
		{
			try
			{
			...
			try
			{
				// Delete old entry from the database
				$db->setQuery(
					$db->getQuery(true)
