  ...
  methods: {
    ...
    itemType() {
      ...
      if (this.item.extension
        && imageExtensions.indexOf(this.item.extension.toLowerCase()) !== -1) {
        return Image;
      }

      // Render video items
      if (this.item.extension
        && videoExtensions.indexOf(this.item.extension.toLowerCase()) !== -1) {
        return Video;
      }
    ...
    handleClick(event) {
      ...
      if (!this.isSelected()) {
        // Unselect all other selected items, if no shift key was pressed during the click event
