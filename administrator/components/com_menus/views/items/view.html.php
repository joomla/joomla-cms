...
class MenusViewItems extends JViewLegacy
{
			...
			switch ($item->type)
			{
				...
				// load language
					$lang->load($item->componentname . '.sys', JPATH_ADMINISTRATOR, null, false, false)
						|| $lang
							->load($item->componentname . '.sys', JPATH_ADMINISTRATOR . '/components/' . $item->componentname, null, false, false)
						|| $lang->load($item->componentname . '.sys', JPATH_ADMINISTRATOR, $lang->getDefault(), false, false)
						|| $lang
							->load($item->componentname . '.sys', JPATH_ADMINISTRATOR . '/components/' . $item->componentname, $lang->getDefault(),
								false, false);
									...
									if (!empty($view[0]['title']))
									{
										...
										if (strpos($vars['layout'], ':') > 0)
										{
											...
											$lang->load('tpl_' . $temp[0] . '.sys', JPATH_SITE, null, false, false)
												...
												|| $lang
													->load('tpl_' . $temp[0] . '.sys', JPATH_SITE . '/templates/' . $temp[0], $lang->getDefault(),
														false, false);
	...
	protected function addToolbar()
	{
		...
		JHtml::_('sidebar.setaction', 'index.php?option=com_menus&view=items');

		JHtml::_('sidebar.addfilter',
			// @todo we need a label here
			'', 'menutype', JHtml::_('select.options', JHtml::_('menu.menus'), 'value', 'text', $this->state->get('filter.menutype')), false);

		JHtml::_('sidebar.addfilter', JText::_('COM_MENUS_OPTION_SELECT_LEVEL'), 'filter_level',
		...
		JHtml::_('sidebar.addfilter', JText::_('JOPTION_SELECT_PUBLISHED'), 'filter_published',
			JHtml::_('select.options', JHtml::_('jgrid.publishedOptions', array('archived' => false)), 'value', 'text',
	...
	{
		return array('a.lft' => JText::_('JGRID_HEADING_ORDERING'), 'a.published' => JText::_('JSTATUS'), 'a.title' => JText::_('JGLOBAL_TITLE'),
			'a.home' => JText::_('COM_MENUS_HEADING_HOME'), 'a.access' => JText::_('JGRID_HEADING_ACCESS'),
			'association' => JText::_('COM_MENUS_HEADING_ASSOCIATION'), 'language' => JText::_('JGRID_HEADING_LANGUAGE'),
