...
class MenuModel extends FormModel
{
	...
	{
		return Factory::getApplication()->getIdentity()->authorise('core.delete', 'com_menus.menu.' . (int) $record->id);
	}
	...
	{
		return Factory::getApplication()->getIdentity()->authorise('core.edit.state', 'com_menus.menu.' . (int) $record->id);
	}
	...
	{
		if (!Factory::getApplication()->getIdentity()->authorise('core.admin', 'com_menus'))
		{
			if (isset($data['rules']))
			{
				unset($data['rules']);
