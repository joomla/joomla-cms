...
$category = Categories::getInstance(str_replace('com_', '', (string) $component) . '.' . $section);

// If there is no category for the component and section, then check the component only
if (!$category) {
    $category = Categories::getInstance(str_replace('com_', '', (string) $component));
}
...
if ((is_countable($this->filterForm->getField('context')->options) ? count($this->filterForm->getField('context')->options) : 0) > 1) {
                ...
                <?php else : ?>
                    ...
                    $user->authorise('core.create', $component)
                        ...
                        <?php echo HTMLHelper::_(
                            ...
                            'collapseModal',
                            ['title' => Text::_('COM_FIELDS_VIEW_FIELDS_BATCH_OPTIONS'), 'footer' => $this->loadTemplate('batch_footer')],
