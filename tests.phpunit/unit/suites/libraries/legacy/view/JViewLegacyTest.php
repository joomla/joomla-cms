...
class JViewLegacyTest extends TestCase
{
	...
	public function test_addPath()
	{
		...
		TestReflection::invoke($this->class, '_addPath', 'helper', realpath(JPATH_ROOT . $ds . 'tests.phpunit'));

		$this->assertAttributeEquals(
			array('helper' => array(realpath(JPATH_ROOT . $ds . 'tests.phpunit') . $ds), 'template' => array(realpath(JPATH_ROOT . $ds . 'libraries') . $ds)),
			'_path',
			$this->class
		);

		TestReflection::invoke($this->class, '_addPath', 'template', realpath(JPATH_ROOT . $ds . 'tests.phpunit'));

		$this->assertAttributeEquals(
			array(
				'helper' => array(realpath(JPATH_ROOT . $ds . 'tests.phpunit') . $ds),
				'template' => array(realpath(JPATH_ROOT . $ds . 'tests.phpunit') . $ds, realpath(JPATH_ROOT . $ds . 'libraries') . $ds)
			),
			'_path',
			$this->class
		);
		...
		$this->assertAttributeEquals(
			array(
				'helper' => array(realpath(JPATH_ROOT . $ds . 'libraries') . $ds, realpath(JPATH_ROOT . $ds . 'tests.phpunit') . $ds),
				'template' => array(realpath(JPATH_ROOT . $ds . 'tests.phpunit') . $ds, realpath(JPATH_ROOT . $ds . 'libraries') . $ds)
			),
			'_path',
			$this->class
