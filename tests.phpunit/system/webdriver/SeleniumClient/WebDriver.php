...
class WebDriver
{
	...
	private function startSession(DesiredCapabilities $desiredCapabilities)
	{
		...
		$results             = $httpClient->setUrl($urlHubFormatted)->setHttpMethod(HttpClient::POST)
		                                  ->setJsonParams($params)->execute();
	...
	public function findElement(By $locator, $polling = false)
	{
		...
		$httpClient = HttpFactory::getClient($this->_environment);
		$results    = $httpClient->setUrl($urlHubFormatted)->setHttpMethod(HttpClient::POST)->setJsonParams($params)
		                         ->setPolling($polling)->execute();
